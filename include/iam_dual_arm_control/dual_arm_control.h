...
#include "dual_pre_grabbing/DualArmPreGrabbing.h"
// #include "dual_pre_grabbing/ObjectToGrab.hpp"
...
public:
  ...
  std::deque<float> _normalForceWindow[NB_ROBOTS];      // Moving window saving the robots' measured normal force to the
                                                        // object's surface [N]
...
public:
  ...
  void get_estimated_state() {
    ...
    wQ_map.row(0) << -_qo(1), _qo(0), -_qo(3), _qo(2);
    wQ_map.row(1) << -_qo(2), _qo(3), _qo(0), -_qo(1);
    wQ_map.row(2) << -_qo(3), -_qo(2), _qo(1), _qo(0);
    _wo = 2.0 * wQ_map * qo_dot;
  }
...
protected:
  ...
  // Vector6f 		_Vee[NB_ROBOTS];
  // Matrix6f 		_tcp_W_EE[NB_ROBOTS];			// Velocity Twist transformation between the robot EE and the tool center
  // point (tcp)
  ...
  // ==============================================
  DualArmPreGrabbing dualPreGrab;
