...
public:
  ...
  bool get_GMR_IO(Eigen::VectorXf Prior,
                  Eigen::MatrixXf Mean_gmm,
    ...
    Output_.setZero();
    //
    E_Cov_I.resize(nI, nI);
    E_Cov_I.setZero();// Expected Input covariance q
    E_Cov_O.resize(nO, nO);
    E_Cov_O.setZero();// Expected Output covariance xi
    //
  ...
  bool compute_gmr_outputs(Eigen::VectorXf Input_,
                           Eigen::VectorXf& Output_,
                           Eigen::MatrixXf& E_Cov_I,
                           Eigen::MatrixXf& E_Cov_O) {

    this->get_GMR_IO(this->Prior_gmm_toss_,
                     this->Mean_gmm_toss_,
                     this->CovMx_gmm_toss_,
                     Input_,
                     Output_,
                     E_Cov_I,
                     E_Cov_O);
  ...
  Eigen::VectorXf get_gmm_vector_of_priors() { return Prior_gmm_toss_; }

  Eigen::MatrixXf get_gmm_matrix_of_means() { return Mean_gmm_toss_; }
  Eigen::MatrixXf get_gmm_matrix_of_covariant_matrices() { return CovMx_gmm_toss_; }
};
...
public:
  ...
  void estimate_2d_throwing_param(ProjectileType type,
  ...
  //
  Eigen::Vector3f
  get_state_variation(float dt, Eigen::Vector3f X, Eigen::Vector3f Xd, Eigen::Vector3f dX_d);//  need to add ds
  ...
  Eigen::Vector2f
  estimateTarget_SimpPathLength_AverageSpeed(Eigen::Vector3f X, Eigen::Vector3f Xd, Eigen::Vector3f aVtarget);//
  Eigen::Vector2f estimate_robot_Lpath_avgVel(Eigen::Vector3f X, Eigen::Vector3f Xd);
  void estimate_landing_state(float T,
                              float g,
                              float mu,
                              Vector7f Xd_land,
                              Vector7f Xrelease,
  ...
  Eigen::Vector3f estimate_target_state_to_go(Eigen::Vector3f Xtarget,
