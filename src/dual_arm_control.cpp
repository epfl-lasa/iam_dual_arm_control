			...
			switch(c){
				...
				case 'a':
							if(_ctrl_mode_conveyor_belt){ _mode_conveyor_belt = 2;
																						...
																						_Regrasping    = true;
																						_regrasp_nb_step = 1;
																						// _dualTaskSelector = VERTICAL_TOSS; //VERTICAL_TOSS
							}
				...
				case 's':
							if(_ctrl_mode_conveyor_belt){ _mode_conveyor_belt = 0; 
																						publish_conveyor_belt_cmds();
																						_isRegraspTossing = !_isRegraspTossing;
								if(_isRegraspTossing){
									_dualTaskSelector = VERTICAL_TOSS; //VERTICAL_TOSS; // toss
									_RegraspCount = 0;
									_regrasp_nb_step = 1;
									// object_._xgp_o[0](2) -= 0.015;
									// object_._xgp_o[1](2) -= 0.015;
									// float alp_reg = 0.20f;
...
void dual_arm_control::computeCommands()
{
  ...
  float y_t_max = 0.10f;   
	...
	Eigen::Vector3f regrasp_release_pos = _xDo_lifting + Eigen::Vector3f(0.0, 0.0, 0.20);
  // ---------------------------------------------------------------------------------------------------------------------------------------
  bool no_dual_mds_method = _old_dual_method;
	...
	bool isRegraspTossing 		= _isRegraspTossing || (_dualTaskSelector == VERTICAL_TOSS);
	...
	// bool RegraspTossing_done= (_release_flag) || ( ((object_._w_H_o.block<3,1>(0,3)-regrasp_release_pos).norm()<=0.05) );
	bool RegraspTossing_done= (_release_flag) || ( (fabs(object_._w_H_o(2,3)-regrasp_release_pos(2)) <=0.05) );
	// ----------------------------------------------------------------------------------------------------------------------------------------
	...
	else 
	{
	    ...
	    //   // --------------------------------------------------------------------------
			...
			// =================================================================================================================
				...
				float regraspOpenOffset = 0.015f;			//
				float regraspStep 			= 0.020f;
				// // constraint on the regrasp step
				// if(regraspStep > ((object_._objectDim(2)/2.0f-0.075) - object_._xgp_o[0](1)) ){
				// 	regraspStep = (object_._objectDim(2)/2.0f-0.075) - object_._xgp_o[0](1);
				// }
				// if(-regraspStep < ((-object_._objectDim(2)/2.0f+0.075) - object_._xgp_o[0](1)) ){
				// 	regraspStep = ((-object_._objectDim(2)/2.0f+0.075) - object_._xgp_o[0](1));
				// }
				...
				Eigen::Vector3f _xqp_o_Open[NB_ROBOTS];
				Eigen::Vector3f _xqp_o_Close[NB_ROBOTS];
				...
				for(int i=0; i<NB_ROBOTS; i++){
				 	...
				 	w_H_release[i] = object_._w_H_o * Utils<float>::pose2HomoMx(_xqp_o_Open[i],  object_._qgp_o[i]);
				 	w_H_regrasp[i] = object_._w_H_o * Utils<float>::pose2HomoMx(_xqp_o_Close[i], object_._qgp_o[i]);
				}
				...
				if(_RegraspCount <= nSamples_regrasp){
					...
					FreeMotionCtrl.computeFastCoordinatedMotion(robot_._w_H_ee,  w_H_regrasp, object_._w_H_o, robot_._Vd_ee, robot_._qd, false);
					// _goToAttractors = true;
				}
				_RegraspCount ++;

				if(_RegraspCount > 2*nSamples_regrasp){

					object_._xgp_o[0] = _xqp_o_Close[0] + Eigen::Vector3f(0.0, -0.0, 0.0);
					object_._xgp_o[1] = _xqp_o_Close[1] + Eigen::Vector3f(0.0,  0.0, 0.0);
			...
			//===================================================================================================================
			{
				...
				if(isRegraspTossing){
					xDesTask = _xDo_lifting + Eigen::Vector3f(0.0, 0.0, 1.0);
					qDesTask = _qDo_lifting;
				}
				// Target to object Orientation Adaptation
				// ----------------------------------------
				...
				if(_dualTaskSelector == VERTICAL_TOSS){
				  _desVtoss   = 0.60f*_desVtoss + 0.40f * _v_max;
				  robot_._Vd_ee[LEFT].head(3)  = robot_._Vd_ee[LEFT].head(3)/( robot_._Vd_ee[LEFT].head(3).norm()+1e-10)  *  _desVtoss;
  				robot_._Vd_ee[RIGHT].head(3) = robot_._Vd_ee[RIGHT].head(3)/(robot_._Vd_ee[RIGHT].head(3).norm()+1e-10) *  _desVtoss;
				}
				...
				//------------------------------
				// _releaseAndretract = dsThrowing.get_release_flag();
				if(  (isPlacing && placing_done) || (isPlaceTossing && placeTossing_done) || (isThrowing && tossing_done) || (isRegraspTossing && RegraspTossing_done)){
					// _releaseAndretract = true;
					if(_dualTaskSelector == 9) _Regrasping = true;
					// _isRegraspTossing = false;
					_dualTaskSelector = 1;
					_RegraspCount = 0;
				}
  ...
  // Extract linear velocity commands and desired axis angle command
	...
	std::cout << " REGRASPING STATUS is -----------> : \t " << _isRegraspTossing << std::endl;
	switch(_dualTaskSelector){
		...
		case 9: std::cout << " DUAL_ARM MODE is -----------------------> : \t " << "VERTICAL-TOSSING" << std::endl; break;
	}
	...
	std::cout << " ///////////////////////////////////////////////////////////////// " <<  std::endl;
	std::cout << "[dual_arm_control]: placing_done  PPPPPPPPP  			\t" << placing_done << std::endl;  //
	std::cout << "[dual_arm_control]: placeTossing_done  PTPTPTPTPT \t" << placeTossing_done << std::endl;  //
	std::cout << "[dual_arm_control]: tossing_done  TTTTTTTTTTTT		\t" << tossing_done << std::endl;  // 
	std::cout << "[dual_arm_control]: _Regrasping  TTTTTTTTTTTT		\t" << _Regrasping << std::endl;  //
	std::cout << "[dual_arm_control]: _regrasp_nb_step  TTTTTTTTTTTT \t" << _regrasp_nb_step << std::endl;  //
	std::cout << "[dual_arm_control]: _RegraspCount  TTTTTTTTTTTT \t" << _RegraspCount << std::endl;  // 
	std::cout << "[dual_arm_control]: DES RELATIVE GRASP POINTS  TTTTTTTTTTTT \t" << object_._xgp_o[0].transpose() << std::endl;  // 
	std::cout << "[dual_arm_control]: DES RELATIVE GRASP POINTS  TTTTTTTTTTTT \t" << object_._xgp_o[1].transpose() << std::endl;  // 
...
void dual_arm_control::prepareCommands(Vector6f Vd_ee[], Eigen::Vector4f qd[], Vector6f V_gpo[])
{
  ...
  // --------------------------------------------------------
  if(!_Regrasping && _goToAttractors && _sensedContact && CooperativeCtrl._ContactConfidence == 1.0f)
  {
  	_nu_Wr0 = 0.60f*_nu_Wr0 + 0.40f; // _nu_Wr0 = 0.80f*_nu_Wr0 + 0.20f;
