...
dual_arm_control::dual_arm_control(ros::NodeHandle& n,
    ...
    nh_(n), loop_rate_(frequency), _dt(1.0f / frequency) {
  // me = this;
  _cycle_count = 0;
  _stop = false;
  ...
  memcpy(_topic_subForceTorqueSensor,
         &topic_sub_ForceTorque_Sensor[0],
         NB_ROBOTS * sizeof *topic_sub_ForceTorque_Sensor);
  //
  //
...
bool dual_arm_control::init() {
  ...
  while (!nh_.getParam("dual_system/simulation", _isSimulation)) {
  ...
  while (!nh_.getParam("dual_system/tool/offset2end_effector/real/left", param_toolOffset_real[LEFT])) {
    ROS_INFO("Waitinng for param:  offset2end_effector/real/left");
  }
  while (!nh_.getParam("dual_system/tool/offset2end_effector/real/right", param_toolOffset_real[RIGHT])) {
    ROS_INFO("Waitinng for param: offset2end_effector/real/right ");
  }
  while (!nh_.getParam("dual_system/tool/offset2end_effector/sim/left", param_toolOffset_sim[LEFT])) {
    ROS_INFO("Waitinng for param: offset2end_effector/sim/left ");
  }
  while (!nh_.getParam("dual_system/tool/offset2end_effector/sim/right", param_toolOffset_sim[RIGHT])) {
    ROS_INFO("Waitinng for param: offset2end_effector/sim/right ");
  }
  while (!nh_.getParam("dual_system/tool/com_position_from_sensor/left", param_com_sensor_pos[LEFT])) {
    ROS_INFO("Waitinng for param: tool com_position_from_sensor/left ");
  }
  while (!nh_.getParam("dual_system/tool/com_position_from_sensor/right", param_com_sensor_pos[RIGHT])) {
    ROS_INFO("Waitinng for param: tool com_position_from_sensor/right ");
  }
  while (!nh_.getParam("dual_system/tool/mass/left", param_toolMass[0])) {
    ROS_INFO("Waitinng for param: tool mass/left ");
  }
  while (!nh_.getParam("dual_system/tool/mass/right", param_toolMass[1])) {
  ...
  while (!nh_.getParam("object/" + param_object_name + "/graspOffset_L", param_graspOffset_L)) {
    ROS_INFO("Waitinng for param: object/graspOffset_L ");
  }
  while (!nh_.getParam("object/" + param_object_name + "/graspOffset_R", param_graspOffset_R)) {
    ROS_INFO("Waitinng for param: object/graspOffset_R ");
  }
  while (!nh_.getParam("object/" + param_object_name + "/dimension", param_objectDim)) {
    ROS_INFO("Waitinng for param: object dimension ");
  }
  while (!nh_.getParam("object/" + param_object_name + "/mass", param_objectMass)) {
  ...
  while (!nh_.getParam("dual_arm_task/reach_to_grasp/impact/desVimp", _desVimp)) {
    ROS_INFO("Waitinng for param: impact/desVimp");
  }
  while (!nh_.getParam("dual_arm_task/reach_to_grasp/desVreach", _desVreach)) {
    ROS_INFO("Waitinng for param: reach_to_grasp/desVreach");
  }
  while (!nh_.getParam("dual_arm_task/reach_to_grasp/impact/impact_direction/friction_angle", _friction_angle)) {
    ROS_INFO("Waitinng for param: impact_direction/friction_angl");
  }
  while (
      !nh_.getParam("dual_arm_task/reach_to_grasp/impact/impact_direction/max_friction_angle", _max_friction_angle)) {
    ROS_INFO("Waitinng for param: impact_direction/max_friction_angle");
  }
  while (!nh_.getParam("dual_arm_task/reach_to_grasp/impact/impact_direction/impact_dir_preset", _impact_dir_preset)) {
    ROS_INFO("Waitinng for param: impact_direction/impact_dir_preset");
  }
  while (!nh_.getParam("dual_arm_task/standby_pose/robot_left/position", param_stanbyPosition[LEFT])) {
    ROS_INFO("Waitinng for param: robot_left/position ");
  }
  while (!nh_.getParam("dual_arm_task/standby_pose/robot_left/orientation", param_stanbyOrientation[LEFT])) {
    ROS_INFO("Waitinng for param: robot_left/orientation ");
  }
  while (!nh_.getParam("dual_arm_task/standby_pose/robot_right/position", param_stanbyPosition[RIGHT])) {
    ROS_INFO("Waitinng for param: robot_right/position ");
  }
  while (!nh_.getParam("dual_arm_task/standby_pose/robot_right/orientation", param_stanbyOrientation[RIGHT])) {
  ...
  while (!nh_.getParam("dual_arm_task/lifting/increment_lift_pos", _increment_lift_pos)) {
    ROS_INFO("Waitinng for param: increment_lift_pos");
  }
  ...
  while (!nh_.getParam("dual_arm_task/placing/height_via_point", _height_via_point)) {
    ROS_INFO("Waitinng for param: placing/height_via_point");
  }
  while (!nh_.getParam("dual_arm_task/tossing/desVtoss", _desVtoss)) {
    ROS_INFO("Waitinng for param: tossing/desVtoss ");
  }
  while (!nh_.getParam("dual_arm_task/tossing/releasePos", param_releasePos)) {
    ROS_INFO("Waitinng for param: tossing/releasePos ");
  }
  while (!nh_.getParam("dual_arm_task/tossing/releaseOrient", param_releaseOrient)) {
    ROS_INFO("Waitinng for param: tossing/releaseOrien ");
  }
  while (!nh_.getParam("dual_arm_task/tossing/releaseLinVel_dir", param_releaseLinVel_dir)) {
    ROS_INFO("Waitinng for param: tossing/releaseLinVel_dir ");
  }
  while (!nh_.getParam("dual_arm_task/tossing/releaseAngVel", param_releaseAngVel)) {
    ROS_INFO("Waitinng for param: tossing/releaseAngVel ");
  }
  while (!nh_.getParam("dual_arm_task/tossing/restPos", param_restPos)) {
    ROS_INFO("Waitinng for param: tossing/restPos ");
  }
  while (!nh_.getParam("dual_arm_task/tossing/restOrient", param_restOrient)) {
    ROS_INFO("Waitinng for param: tossing/restOrient ");
  }
  while (!nh_.getParam("dual_arm_task/tossing/increment_release_pos", _increment_release_pos)) {
  ...
  while (!nh_.getParam("dual_arm_task/dualTaskSelector", _dualTaskSelector)) {
    ROS_INFO("Waitinng for param: dualTaskSelector");
  }
  while (!nh_.getParam("dual_arm_task/old_dual_method", _old_dual_method)) {
  ...
  while (!nh_.getParam("dual_arm_task/isQP_wrench_generation", isQP_wrench_generation)) {
  ...
  while (!nh_.getParam("dual_arm_task/isTargetFixed", _isTargetFixed)) {
  ...
  while (!nh_.getParam("conveyor_belt/nominal_speed", _nominalSpeed_conveyor_belt)) {
    ROS_INFO("Waitinng for param: conveyor_belt/nominal_speed");
  }
  while (!nh_.getParam("conveyor_belt/magnitude_perturbation", _magniture_pert_conveyor_belt)) {
  ...
  Eigen::Vector3f graspOffset_L =
      Eigen::Map<Eigen::VectorXf, Eigen::Unaligned>(param_graspOffset_L.data(), param_graspOffset_L.size());
  Eigen::Vector3f graspOffset_R =
      Eigen::Map<Eigen::VectorXf, Eigen::Unaligned>(param_graspOffset_R.data(), param_graspOffset_R.size());

  _tossVar.release_position =
      Eigen::Map<Eigen::VectorXf, Eigen::Unaligned>(param_releasePos.data(), param_releasePos.size());
  _tossVar.release_orientation =
      Eigen::Map<Eigen::VectorXf, Eigen::Unaligned>(param_releaseOrient.data(), param_releaseOrient.size());
  _tossVar.release_linear_velocity =
      Eigen::Map<Eigen::VectorXf, Eigen::Unaligned>(param_releaseLinVel_dir.data(), param_releaseLinVel_dir.size());
  _tossVar.release_angular_velocity =
      Eigen::Map<Eigen::VectorXf, Eigen::Unaligned>(param_releaseAngVel.data(), param_releaseAngVel.size());
  ...
  _tossVar.rest_orientation =
      Eigen::Map<Eigen::VectorXf, Eigen::Unaligned>(param_restOrient.data(), param_restOrient.size());
  ...
  toolComPositionFromSensor[LEFT] = Eigen::Map<Eigen::VectorXf, Eigen::Unaligned>(param_com_sensor_pos[LEFT].data(),
                                                                                  param_com_sensor_pos[LEFT].size());
  toolComPositionFromSensor[RIGHT] = Eigen::Map<Eigen::VectorXf, Eigen::Unaligned>(param_com_sensor_pos[RIGHT].data(),
                                                                                   param_com_sensor_pos[RIGHT].size());
  xrbStandby[LEFT] = Eigen::Map<Eigen::VectorXf, Eigen::Unaligned>(param_stanbyPosition[LEFT].data(),
                                                                   param_stanbyPosition[LEFT].size());
  qrbStandby[LEFT] = Eigen::Map<Eigen::VectorXf, Eigen::Unaligned>(param_stanbyOrientation[LEFT].data(),
                                                                   param_stanbyOrientation[LEFT].size());
  xrbStandby[RIGHT] = Eigen::Map<Eigen::VectorXf, Eigen::Unaligned>(param_stanbyPosition[RIGHT].data(),
                                                                    param_stanbyPosition[RIGHT].size());
  qrbStandby[RIGHT] = Eigen::Map<Eigen::VectorXf, Eigen::Unaligned>(param_stanbyOrientation[RIGHT].data(),
                                                                    param_stanbyOrientation[RIGHT].size());
  ...
  _sub_object_pose = nh_.subscribe(_topic_pose_object,
                                   1,
                                   &dual_arm_control::objectPoseCallback,
                                   this,
                                   ros::TransportHints().reliable().tcpNoDelay());
  _sub_base_pose[LEFT] =
      nh_.subscribe<geometry_msgs::Pose>(_topic_pose_robot_base[LEFT],
                                         1,
                                         boost::bind(&dual_arm_control::updateBasePoseCallback, this, _1, LEFT),
                                         ros::VoidPtr(),
                                         ros::TransportHints().reliable().tcpNoDelay());
  _sub_ee_pose[LEFT] =
      nh_.subscribe<geometry_msgs::Pose>(_topic_pose_robot_ee[LEFT],
                                         1,
                                         boost::bind(&dual_arm_control::updateEEPoseCallback, this, _1, LEFT),
                                         ros::VoidPtr(),
                                         ros::TransportHints().reliable().tcpNoDelay());
  _sub_ee_velo[LEFT] =
      nh_.subscribe<geometry_msgs::Twist>("/simo_track/robot_left/ee_velo",
                                          1,
                                          boost::bind(&dual_arm_control::updateEETwistCallback, this, _1, LEFT),
                                          ros::VoidPtr(),
                                          ros::TransportHints().reliable().tcpNoDelay());
  _subForceTorqueSensor[LEFT] =
      nh_.subscribe<geometry_msgs::WrenchStamped>(_topic_subForceTorqueSensor[LEFT],
                                                  1,
                                                  boost::bind(&dual_arm_control::updateRobotWrench, this, _1, LEFT),
                                                  ros::VoidPtr(),
                                                  ros::TransportHints().reliable().tcpNoDelay());
  _sub_joint_states[LEFT] =
      nh_.subscribe<sensor_msgs::JointState>("/iiwa1/joint_states",
                                             1,
                                             boost::bind(&dual_arm_control::updateRobotStates, this, _1, LEFT),
                                             ros::VoidPtr(),
                                             ros::TransportHints().reliable().tcpNoDelay());
  _sub_base_pose[RIGHT] =
      nh_.subscribe<geometry_msgs::Pose>(_topic_pose_robot_base[RIGHT],
                                         1,
                                         boost::bind(&dual_arm_control::updateBasePoseCallback, this, _1, RIGHT),
                                         ros::VoidPtr(),
                                         ros::TransportHints().reliable().tcpNoDelay());
  _sub_ee_pose[RIGHT] =
      nh_.subscribe<geometry_msgs::Pose>(_topic_pose_robot_ee[RIGHT],
                                         1,
                                         boost::bind(&dual_arm_control::updateEEPoseCallback, this, _1, RIGHT),
                                         ros::VoidPtr(),
                                         ros::TransportHints().reliable().tcpNoDelay());
  _sub_ee_velo[RIGHT] =
      nh_.subscribe<geometry_msgs::Twist>("/simo_track/robot_right/ee_velo",
                                          1,
                                          boost::bind(&dual_arm_control::updateEETwistCallback, this, _1, RIGHT),
                                          ros::VoidPtr(),
                                          ros::TransportHints().reliable().tcpNoDelay());
  _subForceTorqueSensor[RIGHT] =
      nh_.subscribe<geometry_msgs::WrenchStamped>(_topic_subForceTorqueSensor[RIGHT],
                                                  1,
                                                  boost::bind(&dual_arm_control::updateRobotWrench, this, _1, RIGHT),
                                                  ros::VoidPtr(),
                                                  ros::TransportHints().reliable().tcpNoDelay());
  _sub_joint_states[RIGHT] =
      nh_.subscribe<sensor_msgs::JointState>("/iiwa_blue/joint_states",
                                             1,
                                             boost::bind(&dual_arm_control::updateRobotStates, this, _1, RIGHT),
                                             ros::VoidPtr(),
                                             ros::TransportHints().reliable().tcpNoDelay());
  _sub_target_pose = nh_.subscribe(topic_pose_target,
                                   1,
                                   &dual_arm_control::targetPoseCallback,
                                   this,
                                   ros::TransportHints().reliable().tcpNoDelay());
  //-------------
  // Publishers:
  ...
  _pubDesiredOrientation[LEFT] =
      nh_.advertise<geometry_msgs::Quaternion>("/dual_arm_control/robot_left/desired/ee_orientation", 1);
  _pubDesiredOrientation[RIGHT] =
      nh_.advertise<geometry_msgs::Quaternion>("/dual_arm_control/robot_right/desired/ee_orientation", 1);
  _pubFilteredWrench[LEFT] =
      nh_.advertise<geometry_msgs::WrenchStamped>("/dual_arm_control/robot_left/filteredWrenchLeft", 1);
  _pubFilteredWrench[RIGHT] =
      nh_.advertise<geometry_msgs::WrenchStamped>("/dual_arm_control/robot_right/filteredWrenc0hRight", 1);
  ...
  _pubDesiredVel_Quat[LEFT] =
      nh_.advertise<geometry_msgs::Pose>("/dual_arm_control/iiwa1/vel_quat", 1);// "/passive_control/iiwa1/vel_quat"
  ...
  _pubDistAttractorEe[RIGHT] = nh_.advertise<std_msgs::Float64>("/dual_arm_control/iiwa_blue/error", 1);
  ...
  if (_isSimulation) {
    ...
    _pubAppliedWrench[LEFT] = nh_.advertise<geometry_msgs::Wrench>("/dual_arm_control/iiwa1/applied_wrench", 1);
    _pubAppliedWrench[RIGHT] = nh_.advertise<geometry_msgs::Wrench>("/dual_arm_control/iiwa_blue/applied_wrench", 1);
  }
  _pubApplied_fnornMoment[LEFT] = nh_.advertise<geometry_msgs::Wrench>("/passive_control/iiwa1/ext_nforce_moments", 1);
  _pubApplied_fnornMoment[RIGHT] =
      nh_.advertise<geometry_msgs::Wrench>("/passive_control/iiwa_blue/ext_nforce_moments", 1);
  ...
  //--------------------------------------------------------------------------------------------------------------------
  // Robot, Object and target class objects
  //--------------------------------------------------------------------------------------------------------------------
  //=================
  // robot
  //=================
  ...
  robot_.set_init_parameters(param_toolMass, toolOffsetFromEE, toolComPositionFromSensor, xrbStandby, qrbStandby);
  //=================
  // object_to_grasp
  //=================
  ...
  // =================================================================================
  // instantiation of dual arm pre-grabbing object
  // =================================================================================
  std::string path_to_yaml_file =
      ros::package::getPath(std::string("dual_pre_grabbing")) + "/config/pre_grab_parameters.yaml";
  // get object's hmg Mx from xo and qo
  object_.get_HmgTransform();
  //
  dualPreGrab.init(path_to_yaml_file, object_._w_H_o.cast<double>());
...
void dual_arm_control::run() {
  ...
  dualPreGrab.OutRecord_task.close();
  //
  ros::shutdown();
}
// ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    switch (c) {
      ...
      case 'a':
        if (_ctrl_mode_conveyor_belt) {
          _mode_conveyor_belt = 2;
          publish_conveyor_belt_cmds();
          _startlogging = true;
        } else if (_increment_release_pos)
          _delta_rel_pos(0) -= 0.025f;//_delta_rel_pos(0)  -= 0.05f;  //[m]
        else
          _delta_pos(0) -= 0.01f;
        break;
      case 's':
      ...
      case 'd':
        if (_ctrl_mode_conveyor_belt) {
          _mode_conveyor_belt = 1;
          publish_conveyor_belt_cmds();
        } else if (_increment_release_pos)
          _delta_rel_pos(1) -= 5.0f;//[deg]   _delta_rel_pos(1)  -= 0.025f; //
        else
          _delta_pos(1) -= 0.01f;
        break;
      case 'f':
      ...
      case 'z':
        if (_ctrl_mode_conveyor_belt) _trackingFactor -= 0.01f;
          ...
          _delta_rel_pos(2) -= 5.0f;//[deg]   _delta_rel_pos(2)  -= 0.025f; //
        else
          _delta_pos(2) -= 0.01f;
        break;
      case 'w':
      ...
      case 'r':
        _releaseAndretract = !_releaseAndretract;
        break;
      ...
      case 'o': {
        _isPlaceTossing = !_isPlaceTossing;
        if (_isPlaceTossing) {
...
void dual_arm_control::updatePoses() {
  ...
  //
  if (_increment_release_pos) { this->update_release_position(); }
...
void dual_arm_control::get_pasive_ds_1st_damping() {
  ...
  ros::param::getCached(_dsDampingTopic[LEFT], param_values);
  _d1[LEFT] = param_values[0];
  if (_d1[LEFT] < FLT_EPSILON) _d1[LEFT] = 150.0f;
  ros::param::getCached(_dsDampingTopic[RIGHT], param_values);
  _d1[RIGHT] = param_values[0];
...
void dual_arm_control::computeCommands() {
  ...
  //
  bool isPlacingCommand =
      (_release_flag) || ((object_._w_H_o.block<3, 1>(0, 3) - _xDo_placing).norm() <= 0.07);// 0.07 0.05
  bool isTossingCommand =
      (_release_flag) || ((object_._w_H_o.block<3, 1>(0, 3) - _tossVar.release_position).norm() <= 0.035);
  //
  bool placing_done = (_release_flag) || ((object_._w_H_o.block<3, 1>(0, 3) - _xDo_placing).norm() <= 0.08);// 0.05
  bool placeTossing_done = (_release_flag)
      || (((object_._w_H_o.block<3, 1>(0, 3) - _tossVar.release_position).norm() <= 0.07)
          || ((object_._w_H_o.block<2, 1>(0, 3) - _xDo_placing.head(2)).norm() <= 0.05));
  bool tossing_done =
      (_release_flag) || (((object_._w_H_o.block<3, 1>(0, 3) - _tossVar.release_position).norm() <= 0.035));
  bool isForceDetected =
      (robot_._normalForceAverage[LEFT] > _forceThreshold || robot_._normalForceAverage[RIGHT] > _forceThreshold);
  // ----------------------------------------------------------------------------------------------------------------------------------------
  ...
  Eigen::Vector3f x_i_min =
      this->compute_intercept_with_target(x_origin, target_._xt, target_._vt, -_dual_angular_limit(2));
  ...
  // 		// 	// _dxEE_dual_avg 		 += (0.5f*(_Vee[LEFT].head(3) + _Vee[RIGHT].head(3)).norm() -
  // _dxEE_dual_avg_0)/(_counter_monocycle + 1);
  // 		// 	_dxEE_dual_avg 		 += (Utils<float>::get_abs_3d(_Vee, true).norm() - _dxEE_dual_avg_0)/(_counter_monocycle +
  // 1);
  // 		// 	_xEE_dual 					= Utils<float>::get_abs_3d(_w_H_ee); //0.5f*(robot_._w_H_ee[LEFT].block(0,3,3,1) +
  // robot_._w_H_ee[RIGHT].block(0,3,3,1));
  ...
  Eigen::Vector2f Lp_Va_pred_tgt =
      tossParamEstimator.estimateTarget_SimpPathLength_AverageSpeed(target_._xt, target_._xd_landing, target_._vt);
  //
  ...
  if (!_isPickupSet && !_releaseAndretract) {
    if (_sensedContact && (CooperativeCtrl._ContactConfidence == 1.0)) {
      ...
      object_._x_pickup = object_._xo;
      dsThrowing.set_pickup_object_pose(object_._x_pickup, object_._qo);
    }
  }
  ...
  // ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  {
    ...
    //
    _Vd_o = dsThrowing.apply(object_._xo,
                             object_._qo,
                             object_._vo,
    ...
    for (int i = 0; i < NB_ROBOTS; i++) {
      ...
      _VdImpact[i] = _desVimp * _dirImp[i];                                                  //	impact velocity [LEFT];
      _BasisQ[i] =
          Utils<float>::create3dOrthonormalMatrixFromVector(_dirImp[i]);//  Orthogonal Basis of Modulated Dual-arm DS
    }
    // if( ((_initPoseCount > 50) && ((xt_bar - xt2go_bar).norm() < 0.04f)) && (!_hasCaughtOnce) )// 0.80     // tossing
    // isMotionTriggered
    // if((_initPoseCount > 50) && (fabs(_xo(1)-y_2_go) < 0.02f) && (!_hasCaughtOnce)) 						// 0.80   	//
    // catching
    ...
    {
      std::cout << " ================= RELEASE AND RETRACT  ================= \t " << std::endl;
      ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

      FreeMotionCtrl.computeReleaseAndRetractMotion(robot_._w_H_ee,
                                                    object_._w_H_Dgp,
                                                    object_._w_H_o,
                                                    robot_._Vd_ee,
                                                    robot_._qd,
    ...
    {
      //
      Eigen::Matrix<double, 6, 1> vObj;
      vObj.head(3) = object_._vo.cast<double>();
      vObj.tail(3) = object_._wo.cast<double>();
      Eigen::Matrix4d wHEE[NB_ROBOTS];
      wHEE[LEFT] = robot_._w_H_ee[LEFT].cast<double>();
      wHEE[RIGHT] = robot_._w_H_ee[RIGHT].cast<double>();

      dualPreGrab.generateBackwardTiltingAndCatching(object_._w_H_o.cast<double>(), vObj, wHEE);
      //
      ...
      std::cout << " --------------- \n --------------- \n   " << std::endl;
      std::cout << " --------------- _filteredWrench left  " << robot_._filteredWrench[LEFT].head(3).norm()
                << std::endl;
      std::cout << " --------------- _filteredWrench RIGHT  " << robot_._filteredWrench[RIGHT].head(3).norm()
                << std::endl;

      // if ((robot_._filteredWrench[LEFT].tail(3).norm() + robot_._filteredWrench[RIGHT].tail(3).norm()) >= 0.5) {
      if ((robot_._filteredWrench[LEFT].head(3).norm() > dualPreGrab.getThreshWrenchTilt(LEFT))
          && (robot_._filteredWrench[RIGHT].head(3).norm() > dualPreGrab.getThreshWrenchTilt(RIGHT))) {
        // TODO Both should be touching the box ?
        dualPreGrab.isObjectEEContactTilt = true;
        std::cout << "\033[1;41mbold [dualarm] ------- FORCE OK ------ \033[0m\n" << std::endl;
      } else {
    ...
    else// reaching and constrained motion// ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    {
      ...
      if (isPreGrabbing_) {
        ...
        // object_._w_H_gp[LEFT] = robot_._w_H_ee[LEFT];
        // object_._w_H_gp[RIGHT] = robot_._w_H_ee[RIGHT];
      }

      std::cout << "\033[1;41mbold [dualarm _xDo_lifting \033[0m\n" << _xDo_lifting.transpose() << std::endl;
      ...
      // }
      if (_trackTargetRotation) { this->mirror_target2object_orientation(target_._qt, qDesTask, _dual_angular_limit); }
    ...
    if (isPlacing || isThrowing || isPlaceTossing) {
      ...
      float f_gain = 0.02f;
      float abs_force_correction = _nu_Wr0 * f_gain * 0.5f
    ...
    // ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //
    if (!isContact && (FreeMotionCtrl.a_proximity_ >= 0.2f)) { beta_vel_mod_unfilt = 1.0; }
    // -----------------------------------------------------------------------------------------------------------------------------
    float fil_beta = 0.10;
    ...
    if ((target_._vt.norm() >= 0.05 && (!_releaseAndretract) && (dsThrowing.a_proximity_ <= 0.99f))) {
      robot_._Vd_ee[LEFT].head(3) *=
          _initSpeedScaling * ((float) _adaptationActive * _beta_vel_mod + (1. - (float) _adaptationActive));
      robot_._Vd_ee[RIGHT].head(3) *=
          _initSpeedScaling * ((float) _adaptationActive * _beta_vel_mod + (1. - (float) _adaptationActive));
    }
    ...
    _desired_object_wrench.head(3) =
        -12.64f * (object_._vo - FreeMotionCtrl.get_des_object_motion().head(3)) - object_._objectMass * _gravity;
    _desired_object_wrench.tail(3) = -25.00f * (0.f * object_._wo - FreeMotionCtrl.get_des_object_motion().tail(3));
    ...
    for (int i = 0; i < NB_ROBOTS; i++) { robot_._fxc[i] = 1.0f / _d1[i] * CooperativeCtrl._f_applied[i].head(3); }

    // std::cout << " MEASURED HAND WRENCH _f_applied  LEFT \t " << CooperativeCtrl._f_applied[LEFT].transpose()
    //           << std::endl;
    // std::cout << " MEASURED HAND WRENCH _f_applied  RIGHT \t " << CooperativeCtrl._f_applied[RIGHT].transpose()
    //           << std::endl;
    // std::cout << " MEASURED HAND WRENCH _fxc  LEFT \t " << robot_._fxc[LEFT].transpose() << std::endl;
    // std::cout << " MEASURED HAND WRENCH _fxc  RIGHT \t " << robot_._fxc[RIGHT].transpose() << std::endl;
    //
    // if (isPreGrabbing_) {
  ...
  if (_ctrl_mode_conveyor_belt) {
    _desSpeed_conveyor_belt = (int) (_nominalSpeed_conveyor_belt
                                     + (int) _isDisturbTarget * _magniture_pert_conveyor_belt
                                         * sin((omega_pert + delta_omega_pert) * _dt * _cycle_count));
  }
  ...
  // // std::cout << " [dual_arm_control]: _dirImp[LEFT] \t " << _dirImp[LEFT].transpose()   << " normal LEFT  \t " <<
  // // _n[LEFT].transpose()<< std::endl; std::cout << " [dual_arm_control]: _dirImp[RIGHT] \t " <<
  // // _dirImp[RIGHT].transpose() << " normal RIGHT \t " << _n[RIGHT].transpose()<< std::endl;
  ...
  // std::cout << " PPPPPPPPPPPPPPP _xDo_placing PPPPPPPPPP  is  -----------> : \t " << _xDo_placing.transpose()
  //           << std::endl;
  // std::cout << " TTTTTTTTTTTTTTT _xTarget TTTTTTTTTTT  is  -----------> : \t " << target_._xt.transpose() << std::endl;
  //-------------------------------------
  // -------------------------------------------------------------------------------
  // std::cout << " PPPPPPPPPPPPPPP _dual_PathLen PPPPPPPPPP  is  -----------> : \t " << _dual_PathLen_AvgSpeed(0)
  //           << std::endl;
  // std::cout << " PPPPPPPPPPPPPPP _dual_Path_AvgSpeed(1) PPPPPPPPPP  is  -----------> : \t " << _dual_PathLen_AvgSpeed(1)
  //           << std::endl;
  ...
  // std::cout << "[dual_arm_control]: robot_._w_H_eeStandby[LEFT]: \n" << robot_._w_H_eeStandby[0] << std::endl;  //
  // robot_._w_H_eeStandby std::cout << "[dual_arm_control]: robot_._w_H_eeStandby[RIGHT]: \n" <<
  ...
  std::cout << ".. isObjectEEContactTilt ------> is  \t" << dualPreGrab.isObjectEEContactTilt << std::endl;
...
void dual_arm_control::Keyboard_reference_object_control()// control of attractor position through keyboad
{
  ...
  // // Eigen::Matrix3f RDo_lifting  = Utils<float>::eulerAnglesToRotationMatrix(	_filt_delta_ang(2),
  // _filt_delta_ang(1), _filt_delta_ang(0)); Eigen::Matrix3f RDo_lifting  = Utils<float>::eulerAnglesToRotationMatrix(
  // _filt_delta_ang(0), _filt_delta_ang(1), _filt_delta_ang(2)); _qDo_lifting =
  // Utils<float>::rotationMatrixToQuaternion(RDo_lifting);
  // //
...
void dual_arm_control::mirror_target2object_orientation(Eigen::Vector4f qt,
                                                        Eigen::Vector4f& qo,
  ...
  // qo = Utils<float>::rotationMatrixToQuaternion( Utils<float>::eulerAnglesToRotationMatrix(eAng_t(2), eAng_o(1),
  // eAng_o(0)) );
  qo = Utils<float>::rotationMatrixToQuaternion(
      Utils<float>::eulerAnglesToRotationMatrix(eAng_o(0), eAng_o(1), eAng_t(2)));
  // qo = Utils<float>::rotationMatrixToQuaternion( Utils<float>::eulerAnglesToRotationMatrix(eAng_o(0), eAng_o(1),
  // eAng_o(2)) );
...
{
  // object2grasp.States_Object.pose.head(3) = ioSM->w_H_absF.block<3,3>(0,0) * init_obj_aF +
  // ioSM->w_H_absF.block<3,1>(0,3); //
  ...
  // ang_o += _delta_ang;
  // object_._w_H_o.block<3,3>(0,0) = Utils<float>::eulerAnglesToRotationMatrix(	ang_o(2), ang_o(1), ang_o(0)); // phi
  // theta psi
}
//
Eigen::Vector3f
dual_arm_control::get_impact_direction(Eigen::Vector3f des_object_force, Eigen::Vector3f normal, float coeff_friction) {
  //
  Eigen::Matrix3f R1;
  Eigen::Matrix3f R0;
...
Eigen::Vector3f dual_arm_control::get_object_desired_direction(int task_type, Eigen::Vector3f object_pos) {
  ...
  switch (task_type) {
    case 1:
      des_object_pos = _xDo_lifting;
      break;
    case 2:
      des_object_pos = dsThrowing.Xt_;
      break;
    case 3:
      des_object_pos = dsThrowing.Xt_;
      break;
    case 4:
      des_object_pos = 0.5f * (object_pos + _xDo_placing);
      des_object_pos(2) = _xDo_placing(2) + _height_via_point;
      break;
    case 5:
      des_object_pos = 0.5f * (object_pos + _xDo_placing);
      des_object_pos(2) = _xDo_placing(2) + _height_via_point;
...
void dual_arm_control::update_placing_position(float y_t_min, float y_t_max) {
  _xDo_placing.head(2) =
      _xDo_placing.head(2) + _adaptationActive * _dt * (-5.0f * (_xDo_placing.head(2) - target_._xt.head(2)));
  if ((target_._xt(1) >= y_t_min) && (target_._xt(1) <= y_t_max)) {
    _xDo_placing.head(2) = _xDo_placing.head(2)
        + _adaptationActive * _dt * (target_._vt.head(2) - 5.0f * (_xDo_placing.head(2) - target_._xt.head(2)));
  }
}
...
Eigen::Vector3f dual_arm_control::compute_intercept_with_target(const Eigen::Vector3f& x_pick,
  ...
  // x_i(0) = -1.0f/(std::tan(phi_i) - tan_phi_t) * (x_pick(1) - std::tan(phi_i)*x_pick(0) - (x_target(1) -
  // tan_phi_t*x_target(0))); x_i(1) =  1.0f/(std::tan(phi_i) - tan_phi_t) * ( (std::tan(phi_i)*(x_target(1) -
  // tan_phi_t*x_target(0))) - (tan_phi_t*(x_pick(1) - std::tan(phi_i)*x_pick(0)) ) );
  //
...
void dual_arm_control::estimate_moving_average_ee_speed() {
  ...
  if (_windowSpeedEE.size() < _winLengthAvgSpeedEE) {
    ...
    for (int i = 0; i < _winLengthAvgSpeedEE; i++) { _movingAvgSpeedEE += _windowSpeedEE[i] / _winLengthAvgSpeedEE; }
  }
}
...
void dual_arm_control::find_desired_landing_position(Eigen::Vector3f x_origin,
                                                     bool isPlacing,
                                                     bool isPlaceTossing,
                                                     bool isThrowing) {
  ...
  if (userSelect_) {
    //
    if (isPlacing || isPlaceTossing) { xd_land.head(2) = _xDo_placing.head(2); }
    if (isThrowing) { xd_land.head(2) = _tossVar.release_position.head(2); }
    //
...
void dual_arm_control::find_release_configuration() {// xD_landing
  ...
  if (_feasibleAlgo) {
    ...
    // _tossVar.release_position.head(2) = target_._xd_landing.head(2); //
    //
    Eigen::Vector3f x_release_bar = _tossVar.release_position - object_._x_pickup;
    ...
    _tossVar.release_linear_velocity << _tossVar.release_linear_velocity.head(2).norm() * std::cos(phi_throw_bar),
        _tossVar.release_linear_velocity.head(2).norm() * std::sin(phi_throw_bar), _tossVar.release_linear_velocity(2);
    _tossVar.release_linear_velocity = _desVtoss * _tossVar.release_linear_velocity.normalized();
  } else {// user-defined
...
//
void dual_arm_control::estimate_target_state_to_go(Eigen::Vector2f Lp_Va_pred_bot,
                                                   Eigen::Vector2f Lp_Va_pred_tgt,
                                                   float flytime_obj) {
  // Estimation of the target state-to-go
  // -------------------------------------
  // Target state to go
  target_._xt_state2go = tossParamEstimator.estimate_target_state_to_go(target_._xt,
                                                                        target_._vt,
                                                                        target_._xd_landing,
                                                                        Lp_Va_pred_bot,
                                                                        Lp_Va_pred_tgt,
  ...
  _isMotionTriggered =
      (-xt_bar.dot(target_._vt) > 0) && ((_initPoseCount > 50) && ((xt_bar - xt2go_bar).norm() < 0.04f));
}

void dual_arm_control::compute_adaptation_factors(Eigen::Vector2f Lp_Va_pred_bot,
                                                  Eigen::Vector2f Lp_Va_pred_tgt,
                                                  float flytime_obj) {
  if (_isMotionTriggered) { _isIntercepting = true; }
  //
  float beta_vel_mod_max =
      min(2.0f, min((_v_max / robot_._Vd_ee[LEFT].head(3).norm()), (_v_max / robot_._Vd_ee[RIGHT].head(3).norm())));
  //
  if (_isIntercepting && !_releaseAndretract) {
    ...
    if (_beta_vel_mod_unfilt >= beta_vel_mod_max) { _beta_vel_mod_unfilt = beta_vel_mod_max; }
    // Attractor-based adaptation factor
    //-----------------------------------
    FreeMotionCtrl._activationAperture = _adaptationActive ? 0.5f
            * (std::tanh(_switchSlopeAdapt
                         * ((target_._xd_landing - target_._xt).normalized().dot(target_._vt) - _tol_attractor))
               + 1.0)
                                                           : 1.0f;
  } else {
    _beta_vel_mod_unfilt = 1.0f;
...
void dual_arm_control::updateContactState() {
  ...
  if ((robot_._normalForceAverage[LEFT] > 2.0f || robot_._normalForceAverage[RIGHT] > 2.0f)
      && (_eoC < 0.065f || CooperativeCtrl._ContactConfidence == 1.0f)) {
    _contactState = CONTACT;
    _c = 1.0f;
  } else if (!(robot_._normalForceAverage[LEFT] > 2.0f && robot_._normalForceAverage[RIGHT] > 2.0f) && _eoD < 0.05f
             && _eoC < 0.05f) {
  ...
  // check contact
  _sensedContact =
      ((fabs(robot_._normalForce[LEFT]) >= _forceThreshold) || (fabs(robot_._normalForce[RIGHT]) >= _forceThreshold))
      && (_c == 1.0f);
  // _sensedContact = (fabs(_normalForce[LEFT]) >= _forceThreshold) && (fabs(_normalForce[RIGHT]) >= _forceThreshold) &&
  // (_c == 1.0f);
  std::cerr << "[dual_arm_control]: _sensedContact: " << (int) _sensedContact << " c: " << _c << std::endl;
  std::cerr << "[dual_arm_control]: contact state: " << (int) _contactState << " c: " << _c << std::endl;
  std::cerr << "[dual_arm_control]: robot_._normalForceAverage[LEFT]: " << robot_._normalForceAverage[LEFT]
            << std::endl;
  std::cerr << "[dual_arm_control]: robot_._normalForceAverage[RIGHT]: " << robot_._normalForceAverage[RIGHT]
            << std::endl;
  std::cerr << "[dual_arm_control]: _ContactConfidence: " << CooperativeCtrl._ContactConfidence << std::endl;
  std::cerr << "[dual_arm_control]: _eoD: " << _eoD << std::endl;
  std::cerr << "[dual_arm_control]: _eoC: " << _eoC << std::endl;

  std::cerr << "[dual_arm_control]: robot _w_H_ee LEFT : " << robot_._w_H_ee[LEFT].block(0, 3, 3, 1) << std::endl;
  std::cerr << "[dual_arm_control]: object _w_H_gp LEFT : " << object_._w_H_gp[LEFT].block(0, 3, 3, 1) << std::endl;
  std::cerr << "[dual_arm_control]: robot _w_H_ee RIGHT : " << robot_._w_H_ee[RIGHT].block(0, 3, 3, 1) << std::endl;
  std::cerr << "[dual_arm_control]: object _w_H_gp RIGHT : " << object_._w_H_gp[RIGHT].block(0, 3, 3, 1) << std::endl;
}
...
void dual_arm_control::publishData() {
  ...
  if (_ctrl_mode_conveyor_belt && (fmod(_cycle_count, 30) == 0)) { _pubConveyorBeltSpeed.publish(_speedMessage); }
}
...
void dual_arm_control::saveData() {
  ...
  datalog._OutRecord_pose << robot_._x[LEFT].transpose().format(CSVFormat) << ","
                          << robot_._q[LEFT].transpose().format(CSVFormat) << ",";// left end-effector
  datalog._OutRecord_pose << robot_._x[RIGHT].transpose().format(CSVFormat) << ","
                          << robot_._q[RIGHT].transpose().format(CSVFormat) << ",";// right end-effector
  datalog._OutRecord_pose << object_._xo.transpose().format(CSVFormat) << ","
                          << object_._qo.transpose().format(CSVFormat) << ",";// object
  datalog._OutRecord_pose << object_._w_H_Do(0, 3) << "," << object_._w_H_Do(1, 3) << " , " << object_._w_H_Do(2, 3)
                          << ",";// desired object
  datalog._OutRecord_pose << xgrL.transpose().format(CSVFormat) << "," << qgrL.transpose().format(CSVFormat)
                          << ",";// left  grasping point
  datalog._OutRecord_pose << xgrR.transpose().format(CSVFormat) << "," << qgrR.transpose().format(CSVFormat)
                          << ",";// right grasping point
  datalog._OutRecord_pose << _tossVar.release_position.transpose().format(CSVFormat) << ","
                          << _tossVar.release_orientation.transpose().format(CSVFormat) << ",";// release pose
  datalog._OutRecord_pose << _tossVar.rest_position.transpose().format(CSVFormat) << ","
                          << _tossVar.rest_orientation.transpose().format(CSVFormat) << ",";// rest pose
  datalog._OutRecord_pose << target_._xt.transpose().format(CSVFormat) << ","
                          << target_._qt.transpose().format(CSVFormat) << ",";// target pose
  datalog._OutRecord_pose << target_._xd_landing.transpose().format(CSVFormat) << ","
                          << target_._x_intercept.transpose().format(CSVFormat) << ",";// landing and intercept position
  datalog._OutRecord_pose << target_._xt_state2go.transpose().format(CSVFormat) << ","
                          << _xDo_placing.transpose().format(CSVFormat) << std::endl;// target state to go

  datalog._OutRecord_velo << (float) (_cycle_count * _dt) << ", ";
  datalog._OutRecord_velo << robot_._Vd_ee[LEFT].transpose().format(CSVFormat) << ","
                          << robot_._Vd_ee[RIGHT].transpose().format(CSVFormat) << ",";
  datalog._OutRecord_velo << robot_._Vee[LEFT].transpose().format(CSVFormat) << ","
                          << robot_._Vee[RIGHT].transpose().format(CSVFormat) << ",";
  datalog._OutRecord_velo << robot_._vd[LEFT].transpose().format(CSVFormat) << ","
                          << robot_._vd[RIGHT].transpose().format(CSVFormat) << ",";
  datalog._OutRecord_velo << robot_._omegad[LEFT].transpose().format(CSVFormat) << ","
                          << robot_._omegad[RIGHT].transpose().format(CSVFormat) << ",";
  datalog._OutRecord_velo << object_._vo.transpose().format(CSVFormat) << " , "
                          << object_._wo.transpose().format(CSVFormat) << " , ";
  datalog._OutRecord_velo << _Vd_o.transpose().format(CSVFormat) << " , ";
  datalog._OutRecord_velo << _tossVar.release_linear_velocity.transpose().format(CSVFormat) << " , "
                          << _tossVar.release_angular_velocity.transpose().format(CSVFormat) << " , ";
  ...
  // datalog._OutRecord_tasks   	<< dsThrowing.a_proximity_ << " , " << dsThrowing.a_normal_  << " , " <<
  ...
  datalog._OutRecord_jts_states << robot_._joints_velocities[LEFT].transpose().format(CSVFormat) << " , "
                                << robot_._joints_velocities[RIGHT].transpose().format(CSVFormat) << " , ";
  datalog._OutRecord_jts_states << robot_._joints_accelerations[LEFT].transpose().format(CSVFormat) << " , "
                                << robot_._joints_accelerations[RIGHT].transpose().format(CSVFormat) << " , ";
  datalog._OutRecord_jts_states << robot_._joints_torques[LEFT].transpose().format(CSVFormat) << " , "
