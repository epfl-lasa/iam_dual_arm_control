...
void toss_task_param_estimator::init(std::string file_gmm[],
                                     Eigen::Vector3f x_release,
                                     Eigen::Vector4f q_release,
                                     Eigen::Vector3f v_release,
                                     Eigen::Vector3f w_release) {
  ...
  myGMR.init(file_gmm);
  // datalog.loadGMMParam(file_gmm, Prior_gmm_toss_, Mean_gmm_toss_, CovMx_gmm_toss_);
}

void toss_task_param_estimator::cartesian2planar(Eigen::Vector3f Pos_3d_in,
...
Eigen::Vector3f toss_task_param_estimator::get_state_variation(float dt,
                                                               Eigen::Vector3f X,
                                                               Eigen::Vector3f Xd,
                                                               Eigen::Vector3f dX_d) {// DO REPLACE DS_ROBOT
  //
...
//
void toss_task_param_estimator::get_min_release_speed(Eigen::Vector2f pos_d,
                                                      float& ang_release_i,
                                                      float& v_release_i,
                                                      float& t_flight_i) {
  ...
  t_flight_i = fabs(
      (1. / this->g)
      * (v_release_i * std::sin(theta_i) + std::sqrt(std::sin(theta_i) * std::sin(theta_i) + 2.f * this->g * delta_z)));
}

//
void toss_task_param_estimator::estimate_2d_throwing_param(ProjectileType type,
                                                           Eigen::Vector2f pos_d,
                                                           float& ang_release_i,
                                                           float& v_release_i,
                                                           float& t_flight_i) {
  //
  if (type == PHYS_IDEAL) {
    ...
    this->get_min_release_speed(pos_d,
                                ang_release_i,
                                v_release_i,
                                t_flight_i);// TO DO replace with appropriate function
  } else {                                  //(LEARNED)
...
//
bool toss_task_param_estimator::estimate_tossing_param(ProjectileType type,
                                                       Eigen::Vector3f Pos_landing,
                                                       Eigen::Vector3f Pos_Release) {
  //
  ...
  this->estimate_2d_throwing_param(type,
                                   pos_bar,
                                   ang_release_i,
                                   v_release_i,
                                   t_flight_i);// --> out: ang_release_i, v_release_i, t_flight_i
...
Eigen::Vector3f toss_task_param_estimator::get_release_position() { return x_release_; }
Eigen::Vector4f toss_task_param_estimator::get_release_orientation() { return q_release_; }
Eigen::Vector3f toss_task_param_estimator::get_release_linear_velocity() { return v_release_; }
Eigen::Vector3f toss_task_param_estimator::get_release_angular_velocity() { return w_release_; }
...
void toss_task_param_estimator::projectileMotion2d(float T,
                                                   float g,
                                                   float mu,
  ...
  Eigen::Matrix2f Rt;
  Rt << cos(theta_dd), sin(theta_dd), -sin(theta_dd), cos(theta_dd);
  // projectile dynamics
...
void toss_task_param_estimator::estimate_landing_state(float T,
                                                       float g,
                                                       float mu,
                                                       Vector7f Xd_land,
                                                       Vector7f Xrelease,
                                                       Eigen::Vector3f Vel_feas,
                                                       float& flytime,
                                                       Eigen::Vector3f& Xf_land) {
  //
  ...
  Xf_land << Xland2d(0) * std::cos(Xspherical(2)) + Xrelease(0), Xland2d(0) * std::sin(Xspherical(2)) + Xrelease(1),
      Xland2d(1) + Xrelease(2);
}

Eigen::Vector2f toss_task_param_estimator::estimateTarget_SimpPathLength_AverageSpeed(Eigen::Vector3f Xt,
                                                                                      Eigen::Vector3f Xt_d,
...
Eigen::Vector3f toss_task_param_estimator::estimate_target_state_to_go(Eigen::Vector3f Xtarget,
                                                                       Eigen::Vector3f dX_target,
                                                                       Eigen::Vector3f Xintercept,
                                                                       Eigen::Vector2f Lp_Va_pred_bot,
                                                                       Eigen::Vector2f Lp_Va_pred_tgt,
                                                                       float flytime_obj) {
  float Lptgt2Xintercept = 0.0f;

  if (Lp_Va_pred_bot(1) >= 1e-4) {
    Lptgt2Xintercept = Lp_Va_pred_tgt(1)
        * (Lp_Va_pred_bot(0) / Lp_Va_pred_bot(1)
           + flytime_obj);// avg_speed_target*(robot_PathLength/avg_speed_robot + object_flying_time)
  } else {
