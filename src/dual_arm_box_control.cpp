...
bool dualArmControl::init() {
  ...
  _subForceTorqueSensor[LEFT] =
      nh_.subscribe<geometry_msgs::WrenchStamped>(_topic_subForceTorqueSensor[LEFT],
                                                  ...
                                                  ros::VoidPtr(),
                                                  ros::TransportHints().reliable().tcpNoDelay());
  // _subForceTorqueSensor[LEFT]	= nh_.subscribe(_topic_subForceTorqueSensor[LEFT],1, &dualArmControl::updateRobotWrenchLeft, this, ros::TransportHints().reliable().tcpNoDelay());
  ...
  _subForceTorqueSensor[RIGHT] =
      nh_.subscribe<geometry_msgs::WrenchStamped>(_topic_subForceTorqueSensor[RIGHT],
                                                  ...
                                                  ros::VoidPtr(),
                                                  ros::TransportHints().reliable().tcpNoDelay());
  // _subForceTorqueSensor[RIGHT]	= nh_.subscribe(_topic_subForceTorqueSensor[RIGHT],1, &dualArmControl::updateRobotWrenchRight, this, ros::TransportHints().reliable().tcpNoDelay());
  //-------------
  // Publishers:
  //-------------
  ...
  //
  _sub_N_objects_pose[0] =
      nh_.subscribe<geometry_msgs::Pose>("/simo_track/object1/pose",
                                         1,
                                         boost::bind(&dualArmControl::updateObjectsPoseCallback, this, _1, 0),
                                         ros::VoidPtr(),
                                         ros::TransportHints().reliable().tcpNoDelay());
  _sub_N_objects_pose[1] =
      nh_.subscribe<geometry_msgs::Pose>("/simo_track/object2/pose",
                                         1,
                                         boost::bind(&dualArmControl::updateObjectsPoseCallback, this, _1, 1),
                                         ros::VoidPtr(),
                                         ros::TransportHints().reliable().tcpNoDelay());
  _sub_N_objects_pose[2] =
      nh_.subscribe<geometry_msgs::Pose>("/simo_track/object3/pose",
                                         1,
                                         boost::bind(&dualArmControl::updateObjectsPoseCallback, this, _1, 2),
                                         ros::VoidPtr(),
                                         ros::TransportHints().reliable().tcpNoDelay());
...
void dualArmControl::run() {
  ...
  while (nh_.ok()) {
    ...
    ros::param::getCached("/iiwa1/CustomControllers/controllers/PassiveDS/params", param_values);
    _d1[LEFT] = param_values[0];
    if (_d1[LEFT] < FLT_EPSILON) _d1[LEFT] = 150.0f;
    ros::param::getCached("/iiwa_blue/CustomControllers/controllers/PassiveDS/params", param_values);
...
void dualArmControl::updatePoses() {
  ...
  if (_initPoseCount < 5) {
    _w_H_eeStandby[LEFT] = _w_H_rb[LEFT]
        * Utils<float>::pose2HomoMx(_xrbStandby[LEFT], _qrbStandby[LEFT]);// WITH EE pose wrt. the world TO BE CHANGED
    _w_H_eeStandby[RIGHT] = _w_H_rb[RIGHT]
        * Utils<float>::pose2HomoMx(_xrbStandby[RIGHT], _qrbStandby[RIGHT]);// WITH EE pose wrt. the world
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////
  for (int k = 0; k < NB_OBJECTS; k++) { _w_H_No[k] = Utils<float>::pose2HomoMx(_xNo[k], _qNo[k]); }
  ...
  if (CooperativeCtrl._ContactConfidence == 1.0) {
    for (int k = 0; k < NB_ROBOTS; k++) {
      ...
      _w_H_Dgp[k].block(0, 0, 3, 3) =
          _w_H_Do.block(0, 0, 3, 3) * Utils<float>::pose2HomoMx(_xgp_o[k], _qgp_o[k]).block(0, 0, 3, 3);
    }
  }
  ...
  Eigen::Vector3f t_o_absEE = _w_H_abs_o.block(0, 3, 3, 1)
      - 0.5f * (_w_H_ee[LEFT].block(0, 3, 3, 1) + _w_H_ee[RIGHT].block(0, 3, 3, 1));// _w_H_abs_o
  ...
  std::cout << "[dualArmControl]: _w_H_abs_o: \n" << _w_H_abs_o << std::endl;
  ...
  std::cout << "[dualArmControl]: _xDo: \t" << _xDo.transpose() << std::endl;
  std::cout << "[dualArmControl]: _qDo: \n" << _qDo.transpose() << std::endl;
  // std::cout << "[dualArmControl]: _w_H_ee[LEFT]: \n" <<  _w_H_ee[0] << std::endl;
  std::cout << "[dualArmControl]: _w_H_gp[LEFT]: \n" << _w_H_gp[0] << std::endl;
  // std::cout << "[dualArmControl]: _w_H_ee[RIGHT]: \n" << _w_H_ee[1] << std::endl;
  std::cout << "[dualArmControl]: _w_H_gp[RIGHT]: \n" << _w_H_gp[1] << std::endl;
  // std::cout << "[dualArmControl]: _w_H_eeStandby[LEFT]: \n" <<  _w_H_eeStandby[0] << std::endl;
  // std::cout << "[dualArmControl]: _w_H_eeStandby[RIGHT]: \n" << _w_H_eeStandby[1] << std::endl;

  std::cout << "[dualArmControl]:  ddddddddddddddddd  _eoD: \t" << _eoD << std::endl;
  std::cout << "[dualArmControl]:  ddddddddddddddddd  _eoC: \t" << _eoC << std::endl;
  std::cout << "[dualArmControl]: _releaseAndretract: \n" << _releaseAndretract << std::endl;

  std::cout << " ------------------------------------------------------------------- " << std::endl;
  // std::cout << "[dualArmControl]: normal to Surface[LEFT]: \t" << _n[0].transpose() << std::endl;
  // std::cout << "[dualArmControl]: normal to Surface[RIGHT]: \t" << _n[1].transpose() << std::endl;
  std::cout << "[dualArmControl]: _normalForce[LEFT]: \t" << _normalForce[0] << std::endl;
  std::cout << "[dualArmControl]: _normalForce[RIGHT]: \t" << _normalForce[1] << std::endl;
  std::cout << " ------------------------------------------------------------------- " << std::endl;

  std::cout << "[dualArmControl]: FFFFFFF  filteredWrench[LEFT]: \t" << _filteredWrench[0].transpose() << std::endl;
  std::cout << "[dualArmControl]: FFFFFFF  filteredWrench[RIGHT]: \t" << _filteredWrench[1].transpose() << std::endl;

  // std::cout << "[dualArmControl]: DESIRED FORCE[LEFT]: \t"  << _Fd[0] << std::endl;
  // std::cout << "[dualArmControl]: DESIRED FORCE[RIGHT]: \t" << _Fd[1] << std::endl;
}
void dualArmControl::computeCommands() {
  ...
  // check contact
  _sensedContact =
      (fabs(_normalForce[LEFT]) >= _forceThreshold) && (fabs(_normalForce[RIGHT]) >= _forceThreshold) && (_c == 1.0f);
  //
  Vector6f desired_object_wrench_;
  desired_object_wrench_.setZero();
  desired_object_wrench_.head(3) =
      -20.0f * (_w_H_o.block(0, 3, 3, 1) - _w_H_Do.block(0, 3, 3, 1)) - _objectMass * _gravity;
  ...
  if (_goHome) {
    ...
    //  _vd[i]  = _vd[i] + _fxc[i];
    // }
    for (int i = 0; i < NB_ROBOTS; i++) { _fxc[i] = 1.0f / _d1[i] * CooperativeCtrl._f_applied[i].head(3); }
  }
  ...
  std::cout << "[dualArmControl]: _Vd_ee[LEFT]:  \t" << _Vd_ee[LEFT].transpose() << std::endl;
  std::cout << "[dualArmControl]: _Vd_ee[RIGHT]: \t" << _Vd_ee[RIGHT].transpose() << std::endl;
  std::cout << " vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv " << std::endl;
  std::cout << "[dualArmControl]: _vd[LEFT]:  \t" << _vd[LEFT].transpose() << std::endl;
  std::cout << "[dualArmControl]: _vd[RIGHT]: \t" << _vd[RIGHT].transpose() << std::endl;
  std::cout << " ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ " << std::endl;
}
...
void dualArmControl::updateObjectsPoseCallback(const geometry_msgs::Pose::ConstPtr& msg,
                                               int k)// updateObjectsPoseCallback
{
...
void dualArmControl::updateRobotWrench(const geometry_msgs::WrenchStamped::ConstPtr& msg, int k) {
  ...
  if (!_wrenchBiasOK[k]) {
    ...
    if (_wrenchCount[k] == NB_FT_SENSOR_SAMPLES) {
      ...
      std::cerr << "[dualArmControl]: Bias " << k << ": " << _wrenchBias[k].transpose() << std::endl;
    }
  }
  ...
  std::cout << "[dualArmControl]: FFFFFFFF filteredWrench[" << k << "]: \t" << _filteredWrench[k].transpose()
            << std::endl;
}

void dualArmControl::updateRobotWrenchLeft(const geometry_msgs::WrenchStamped::ConstPtr& msg) {
  Eigen::Matrix<float, 6, 1> raw;
  raw(0) = msg->wrench.force.x;
  raw(1) = msg->wrench.force.y;
  raw(2) = msg->wrench.force.z;
  ...
  std::cout << "[dualArmControl]: FFFFFFFF filteredWrench[" << LEFT << "]: \t" << _filteredWrench[LEFT].transpose()
            << std::endl;
}

void dualArmControl::updateRobotWrenchRight(const geometry_msgs::WrenchStamped::ConstPtr& msg) {
  Eigen::Matrix<float, 6, 1> raw;
  raw(0) = msg->wrench.force.x;
  raw(1) = msg->wrench.force.y;
  raw(2) = msg->wrench.force.z;
  ...
  std::cout << "[dualArmControl]: FFFFFFFF filteredWrench[" << RIGHT << "]: \t" << _filteredWrench[RIGHT].transpose()
            << std::endl;
}
...
void dualArmControl::updateContactState() {
  ...
  std::cerr << "[dualArmControl]: _normalForceAverage[LEFT]: " << _normalForceAverage[LEFT] << std::endl;
  std::cerr << "[dualArmControl]: _normalForceAverage[RIGHT]: " << _normalForceAverage[RIGHT] << std::endl;
}
...
void dualArmControl::getGraspPointsVelocity() {
...
void dualArmControl::Keyboard_reference_object_control() {
...
// //
void dualArmControl::Keyboard_virtual_object_control() {
