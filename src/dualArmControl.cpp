...
dualArmControl::dualArmControl(ros::NodeHandle& n,
  ...
  // timeStartRun_ = ros::Time::now().toSec();
  _release_flag = false;
  _startlogging = false;
  _isPickupSet = false;
  _dualTaskSelector = 1;
...
bool dualArmControl::initRosSubscribers() {
  ...
  ros::Subscriber subObjectPose = nh_.subscribe(_topic_pose_object,
                                                1,
  ...
  ros::Subscriber subJointStateLeft =
      nh_.subscribe<sensor_msgs::JointState>("/iiwa1/joint_states",
                                             ...
                                             ros::VoidPtr(),
                                             ros::TransportHints().reliable().tcpNoDelay());
  ros::Subscriber subBasePoseLeftRight =
...
bool dualArmControl::initRosPublisher() {
  pubTSCommands_[LEFT] = nh_.advertise<std_msgs::Float64MultiArray>(_topic_ee_commands[LEFT], 1);  // commands
  pubTSCommands_[RIGHT] = nh_.advertise<std_msgs::Float64MultiArray>(_topic_ee_commands[RIGHT], 1);// commands
  pubDesiredOrientation_[LEFT] =
      nh_.advertise<geometry_msgs::Quaternion>("/dual_arm_control/robot_left/desired/ee_orientation", 1);
  pubDesiredOrientation_[RIGHT] =
      nh_.advertise<geometry_msgs::Quaternion>("/dual_arm_control/robot_right/desired/ee_orientation", 1);
  pubFilteredWrench_[LEFT] =
      nh_.advertise<geometry_msgs::WrenchStamped>("/dual_arm_control/robot_left/filteredWrenchLeft", 1);
  pubFilteredWrench_[RIGHT] =
      nh_.advertise<geometry_msgs::WrenchStamped>("/dual_arm_control/robot_right/filteredWrenc0hRight", 1);
  pubNormalForce_[LEFT] = nh_.advertise<std_msgs::Float64>("/dual_arm_control/robot_left/normalForceLeft", 1);
  pubNormalForce_[RIGHT] = nh_.advertise<std_msgs::Float64>("/dual_arm_control/robot_right/normalForceRight", 1);
  // Desired command for the dual
  pubDesiredVelQuat_[LEFT] =
      nh_.advertise<geometry_msgs::Pose>("/dual_arm_control/iiwa1/vel_quat", 1);// "/passive_control/iiwa1/vel_quat"
  pubDesiredVelQuat_[RIGHT] = nh_.advertise<geometry_msgs::Pose>("/dual_arm_control/iiwa_blue/vel_quat", 1);
  pubDesiredTwist_[LEFT] = nh_.advertise<geometry_msgs::Twist>("/dual_arm_control/robot_left/desired/ee_velocity",
                                                               1);// /passive_control/iiwa1/des_twist
  pubDesiredTwist_[RIGHT] = nh_.advertise<geometry_msgs::Twist>("/dual_arm_control/robot_right/desired/ee_velocity", 1);
  // pubDesiredTwist_[LEFT] 		= nh_.advertise<geometry_msgs::Twist>("/passive_control/iiwa1/des_twist", 1);  //  "/dualArmControl/robot_right/desired/ee_velocity"
  // pubDesiredTwist_[RIGHT] 	 	= nh_.advertise<geometry_msgs::Twist>("/passive_control/iiwa_blue/des_twist", 1);
  pubAttractor_[LEFT] = nh_.advertise<geometry_msgs::Pose>("/dual_arm_control/iiwa1/attractor", 1);
  pubAttractor_[RIGHT] = nh_.advertise<geometry_msgs::Pose>("/dual_arm_control/iiwa_blue/attractor", 1);
  pubDistAttractorEE_[LEFT] = nh_.advertise<std_msgs::Float64>("/dual_arm_control/iiwa1/error", 1);
  pubDistAttractorEE_[RIGHT] = nh_.advertise<std_msgs::Float64>("/dual_arm_control/iiwa_blue/error", 1);
  pubNormLinVel_[LEFT] = nh_.advertise<std_msgs::Float64>("/dual_arm_control/iiwa_left/lin_vel_norm", 1);
  pubNormLinVel_[RIGHT] = nh_.advertise<std_msgs::Float64>("/dual_arm_control/iiwa_right/lin_vel_norm", 1);
  //
  pubAppliedWrench_[LEFT] = nh_.advertise<geometry_msgs::Wrench>("/dual_arm_control/robot_left/applied_wrench", 1);
  pubAppliedWrench_[RIGHT] = nh_.advertise<geometry_msgs::Wrench>("/dual_arm_control/robot_right/applied_wrench", 1);
  pubAppliedFNormMoment_[LEFT] = nh_.advertise<geometry_msgs::Wrench>("/passive_control/iiwa1/ext_nforce_moments", 1);
  pubAppliedFNormMoment_[RIGHT] =
      nh_.advertise<geometry_msgs::Wrench>("/passive_control/iiwa_blue/ext_nforce_moments", 1);

  pubConveyorBeltMode_ = nh_.advertise<std_msgs::Int32>("/conveyor_belt/desired_mode", 1);
  pubConveyorBeltSpeed_ = nh_.advertise<std_msgs::Int32>("/conveyor_belt/desired_speed", 1);
}
...
bool dualArmControl::init() {
  ...
  // Data recording:
  //=========================================
  datalog.datalog_init(ros::package::getPath(std::string("dualArmControl")) + "/Data");

  // signal(SIGINT,dualArmControl::stopNode);
  // //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  timeStartRun_ = ros::Time::now().toSec();
  //
  if (nh_.ok()) {
...
// void dualArmControl::stopNode(int sig)
// {
//   me->_stop = true;
// }
...
void dualArmControl::computeCommands() {
  ...
  // Application
  // ===========================================================================================================
  if ((!_releaseAndretract) && (fmod(cycleCount_, 20) == 0)) {
  ...
  // ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  {
    ...
    //======================================================================================================================================
    {
      ...
      //
      if (FreeMotionCtrl.a_proximity_ >= 0.2f) { _beta_vel_mod_unfilt = 1.0; }
    }
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  std::cout << "[dualArmControl]:  ------------- _sensedContact: \t" << _sensedContact << std::endl;
  ...
  // std::cout << " [dualArmControl]: _dirImp[LEFT] \t " << _dirImp[LEFT].transpose()   << " normal LEFT  \t " << _n[LEFT].transpose()<< std::endl;
  // std::cout << " [dualArmControl]: _dirImp[RIGHT] \t " << _dirImp[RIGHT].transpose() << " normal RIGHT \t " << _n[RIGHT].transpose()<< std::endl;
  ...
  std::cout << "[dualArmControl]: _desired_object_wrench:  WWWWWWWW  \t" << _desired_object_wrench.transpose()
            << std::endl;//
  //---------------------------------------------------------------------------------------------------------------------------------------------
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void dualArmControl::update_release_position() {
  ...
  release_pos.toCartesian(pos_xo);
...
void dualArmControl::update_placing_position(float y_t_min, float y_t_max) {
  _xDo_placing.head(2) =
      _xDo_placing.head(2) + _adaptationActive * dt_ * (-5.0f * (_xDo_placing.head(2) - target_._xt.head(2)));
  if ((target_._xt(1) >= y_t_min) && (target_._xt(1) <= y_t_max)) {
    _xDo_placing.head(2) = _xDo_placing.head(2)
        + _adaptationActive * dt_ * (target_._vt.head(2) - 5.0f * (_xDo_placing.head(2) - target_._xt.head(2)));
  }
}
...
void dualArmControl::updateContactState() {
  ...
  std::cerr << "[dualArmControl]: contact state: " << (int) _contactState << " c: " << _c << std::endl;
...
void dualArmControl::publish_commands() {
  ...
  //
  pubTSCommands_[LEFT].publish(_pubVelo[LEFT]);
  pubTSCommands_[RIGHT].publish(_pubVelo[RIGHT]);
...
void dualArmControl::publishData() {
  ...
  if (_ctrl_mode_conveyor_belt && (fmod(cycleCount_, 30) == 0)) { pubConveyorBeltSpeed_.publish(_speedMessage); }
}
...
void dualArmControl::saveData() {
  ...
  // if(_startlogging)
  // {
  datalog._OutRecord_pose << (float) (cycleCount_ * dt_) << ", ";// cycle time
  ...
  datalog._OutRecord_velo << (float) (cycleCount_ * dt_) << ", ";
  ...
  datalog._OutRecord_efforts << (float) (cycleCount_ * dt_) << ", ";
  ...
  datalog._OutRecord_tasks << (float) (cycleCount_ * dt_) << ", ";
  ...
  datalog._OutRecord_tasks << _goHome << " , " << _goToAttractors << " , " << _releaseAndretract << " , " << _isThrowing
                           << " , " << _isPlacing << " , " << _c << " , ";//CooperativeCtrl._ContactConfidence << " , ";
  ...
  //
  datalog._OutRecord_jts_states << (float) (cycleCount_ * dt_) << ", ";
