...
void dualArmFreeMotionController::computeCoordinatedMotion(Eigen::Matrix4f w_H_ee[], Eigen::Matrix4f w_H_gp[],
  ...
  // =======================================
  Eigen::Vector3f d_p_abs =
      reachable_p * w_H_ap.block<3, 1>(0, 3) + (1.0f - reachable_p) * w_H_ar_stb.block<3, 1>(0, 3);
  ...
  Eigen::Matrix3f d_R_abs =
      reachable_p * w_H_ap.block<3, 3>(0, 0) + (1.0f - reachable_p) * w_H_ar_stb.block<3, 3>(0, 0);
  Eigen::Matrix4f w_H_ar_t = w_H_ar;
  w_H_ar_t.block<3, 3>(0, 0) =
      Utils<float>::getCombinedRotationMatrix(cpl_abs, w_H_ar.block<3, 3>(0, 0), d_R_abs);// desired
  ...
  Eigen::Matrix3f jacMuTheta_abs =
      Utils<float>::getMuThetaJacobian(d_H_c_abs.block<3, 3>(0, 0)) * w_H_ar.block<3, 3>(0, 0).transpose();
  ...
  // Coupling the orientation with the position error
  Eigen::Matrix3f d_R_rel =
      reachable_p * (coord_abs * lp_H_rp.block<3, 3>(0, 0) + (1.0f - coord_abs) * lp_H_rp_pgrasp.block<3, 3>(0, 0))
      + (1.0f - reachable_p) * lr_H_rr_stb.block<3, 3>(0, 0);

  Eigen::Matrix4f lr_H_rr_t = lr_H_rr;
  lr_H_rr_t.block<3, 3>(0, 0) =
      Utils<float>::getCombinedRotationMatrix(coord_abs, lr_H_rr.block<3, 3>(0, 0), d_R_rel);// desired
  // relative transformation
  ...
  // ///////////////////////////////////////////////////////////////////////////////////////////
  // relative transformation
  lr_H_rr_t.block<3, 3>(0, 0) = Utils<float>::quaternionToRotationMatrix(qd[LEFT]).transpose()
  ...
  // 3D Orientation Jacobian
  Eigen::Matrix3f jacMuTheta_rel = Utils<float>::getMuThetaJacobian(d_H_c_rel.block<3, 3>(0, 0))
      * w_H_ee[RIGHT].block<3, 3>(0, 0).transpose();// wrt. the world
  ...
  float cp_ap =
      computeCouplingFactor(o_error_pos_abs_paral, 50.0f, 0.12f, 1.0f, true);// 50.0f, 0.05f, 2.8f  (0.04f) (0.02f)

  // position error accounting for the reachability of the target
  // Eigen::Vector3f d_p_rel = reachable_p *(cpl_rel*cp_ap* lp_H_rp.block<3,1>(0,3) + (1.0f-cpl_rel*cp_ap)
  // *lp_H_rp_pgrasp.block<3,1>(0,3)) + (1.0f-reachable_p) * lr_H_rr_stb.block<3,1>(0,3); // TBC
  Eigen::Vector3f d_p_rel =
      cpl_rel * (cp_ap * lp_H_rp.block<3, 1>(0, 3) + (1.0f - cp_ap) * lp_H_rp_pgrasp.block<3, 1>(0, 3))
  ...
  // std::cout << "[dual_arm_control]: CCCCCCCCCCC TANH: \t" <<  1.0f-std::tanh(3.0f*_error_rel.head(3).norm()) <<
  // std::endl; std::cout << "[dual_arm_control]: CCCCCCCCCCC cpl_rel: \t" <<  cpl_rel << std::endl; std::cout <<
  // "[dual_arm_control]: Absolute Error: \t" <<  _V_abs.transpose() << std::endl; std::cout << "[dual_arm_control]:
  // Absolute Velo: \t" <<  _V_abs.transpose() << std::endl; std::cout << "[dual_arm_control]: Relative Velo: \t" <<
  // _V_rel.transpose() << std::endl;

  // // Computation of desired orientation
  // // this->computeDesiredOrientation(coord_abs, w_H_ee, w_H_gp, w_H_o, qd, isOrient3d);
  // // this->computeDesiredOrientation(1.0f-std::tanh(3.0f*_error_rel.head(3).norm()), w_H_ee, w_H_gp, w_H_o, qd,
...
void dualArmFreeMotionController::computeConstrainedMotion(Eigen::Matrix4f w_H_ee[], Eigen::Matrix4f w_H_gp[],
  ...
  // position error accounting for the reachability of the target
  Eigen::Vector3f d_p_rel =
      reachable_p * (cpl_rel * lp_H_rp.block<3, 1>(0, 3) + (1.0f - cpl_rel) * lp_H_rp_pgrasp.block<3, 1>(0, 3))
...
void dualArmFreeMotionController::computeAsyncMotion(Eigen::Matrix4f w_H_ee[], Eigen::Matrix4f w_H_gp[],
  ...
  for (int k = 0; k < NB_ROBOTS; k++) {
    ...
    Eigen::Vector3f d_p_ee =
        reachable_p * w_H_gp[k].block<3, 1>(0, 3) + (1.0f - reachable_p) * this->_w_H_eeStandby[k].block<3, 1>(0, 3);
    Eigen::Matrix3f d_R_ee =
        reachable_p * w_H_gp[k].block<3, 3>(0, 0) + (1.0f - reachable_p) * this->_w_H_eeStandby[k].block<3, 3>(0, 0);
    Eigen::Matrix4f w_H_ee_t = w_H_ee[k];
    w_H_ee_t.block<3, 3>(0, 0) =
        Utils<float>::getCombinedRotationMatrix(1.0f, w_H_ee[k].block<3, 3>(0, 0), d_R_ee);// desired
    ...
    Eigen::Matrix3f jacMuTheta_ee =
        Utils<float>::getMuThetaJacobian(d_H_c_ee.block<3, 3>(0, 0)) * w_H_ee[k].block<3, 3>(0, 0).transpose();
    // ---------------------------------
...
void dualArmFreeMotionController::computeReleaseAndRetractMotion(Eigen::Matrix4f w_H_ee[], Eigen::Matrix4f w_H_gp[],
  ...
  Eigen::Matrix4f lr_H_rr_t = lr_H_rr;
  lr_H_rr_t.block<3, 3>(0, 0) =
      Utils<float>::getCombinedRotationMatrix(1.0f, lr_H_rr.block<3, 3>(0, 0), d_R_rel);// desired
  // relative transformation
  // Eigen::Matrix4f d_H_c_rel = lr_H_rr_t.inverse() * lr_H_rr;  // expressed in the left hand frame
...
void dualArmFreeMotionController::generatePlacingMotion(Eigen::Matrix4f w_H_ee[], Eigen::Matrix4f w_H_gp[],
  ...
  Eigen::Vector3f d_pos_obj =
      sat_cpl_z * (cpl_Doxy * w_H_Do.block<3, 1>(0, 3) + (1.f - cpl_Doxy) * w_H_Do_z.block<3, 1>(0, 3))
      + (1.f - sat_cpl_z) * w_H_o_z.block<3, 1>(0, 3);
  Eigen::Matrix3f jacMuTheta_obj = Utils<float>::getMuThetaJacobian(d_H_c_obj.block<3, 3>(0, 0))
      * w_H_o.block<3, 3>(0, 0).transpose();// 3D Orientation Jacobian
  // pose error
  ...
  Eigen::Matrix4f d_H_c_rel = lr_H_rr_t.inverse() * lr_H_rr;// expressed in the left hand frame
  Eigen::Matrix3f jacMuTheta_rel = Utils<float>::getMuThetaJacobian(d_H_c_rel.block<3, 3>(0, 0))
      * w_H_ee[RIGHT].block<3, 3>(0, 0).transpose();    //   // 3D Orientation Jacobian wrt. the world
  ...
  for (int i = 0; i < NB_ROBOTS; i++) {
    ...
    skew_Mx_[i] << 0.0f, -t(2), t(1), t(2), 0.0f, -t(0), -t(1), t(0), 0.0f;
  }
...
void dualArmFreeMotionController::computeCoordinatedMotion2(Eigen::Matrix4f w_H_ee[], Eigen::Matrix4f w_H_gp[],
  ...
  lp_H_rp_momentum.block(0, 3, 3, 1) =
      comb_coef * lp_H_rp_pgrasp.block(0, 3, 3, 1) + (1. - comb_coef) * lr_H_rr_stb.block(0, 3, 3, 1);

  w_H_ap_momentum = w_H_ap;
  w_H_ap_momentum.block(0, 3, 3, 1) =
      comb_coef * w_H_ap.block(0, 3, 3, 1) + (1. - comb_coef) * w_H_ar_stb.block(0, 3, 3, 1);
  ...
  Eigen::Vector3f d_p_abs = _activationAperture * reachable_p * w_H_ap.block<3, 1>(0, 3)
      + (1.0f - _activationAperture * reachable_p)
          * (_activationAperture * w_H_ar_stb.block<3, 1>(0, 3)
             + (1. - _activationAperture) * w_H_ap_momentum.block<3, 1>(0, 3));
  _error_abs.head(3) = w_H_ar.block<3, 1>(0, 3) - d_p_abs;
  ...
  Eigen::Vector3f o_error_pos_abs = w_H_o.block<3, 3>(0, 0).transpose() * _error_abs.head(3);
  Eigen::Vector3f o_error_pos_abs_paral = Eigen::Vector3f(o_error_pos_abs(0), 0.0f, o_error_pos_abs(2));
  float cp_ap = _activationAperture
      * computeCouplingFactor(o_error_pos_abs_paral, 50.0f, 0.06f, 1.0f, true);// 50.0f, 0.12f, 1.0f  (0.04f) (0.02f)

  // position error accounting for the reachability of the target
  // Eigen::Vector3f d_p_rel = reachable_p *(cpl_rel*cp_ap* lp_H_rp.block<3,1>(0,3) + (1.0f-cpl_rel*cp_ap)
  // *lp_H_rp_pgrasp.block<3,1>(0,3)) + (1.0f-reachable_p) * lr_H_rr_stb.block<3,1>(0,3); // TBC
  Eigen::Vector3f d_p_rel = cpl_rel
          * (cp_ap * lp_H_rp.block<3, 1>(0, 3)
             + (1.0f - cp_ap)
                 * (_activationAperture * lp_H_rp_pgrasp.block<3, 1>(0, 3)
                    + (1. - _activationAperture) * lp_H_rp_momentum.block<3, 1>(0, 3)))
      + (1.0f - cpl_rel) * lr_H_rr_stb.block<3, 1>(0, 3);// TBC
  ...
  // 3D Orientation Jacobian
  Eigen::Matrix3f jacMuTheta_l = Utils<float>::getMuThetaJacobian(d_H_c_l.block<3, 3>(0, 0))
      * w_H_ee[LEFT].block<3, 3>(0, 0).transpose();// wrt. the world
  Eigen::Matrix3f jacMuTheta_r = Utils<float>::getMuThetaJacobian(d_H_c_r.block<3, 3>(0, 0))
      * w_H_ee[RIGHT].block<3, 3>(0, 0).transpose();// wrt. the world
...
void dualArmFreeMotionController::computeCoordinatedMotion3(Eigen::Matrix4f w_H_ee[], Eigen::Matrix4f w_H_gp[],
  ...
  // Eigen::Vector3f pvo_star = (1.f - reachable_p)* _x_intercept + reachable_p* w_H_o.block<3,1>(0,3);           //
  // _go2object
  ...
  bool object_between_ee = ((w_H_ee[LEFT](1, 3) - (w_H_o(1, 3) - _objectDim(1) / 2.f) < 0.0f)
                            && (w_H_ee[RIGHT](1, 3) - (w_H_o(1, 3) + _objectDim(1) / 2.f) > 0.0f));
  Eigen::MatrixXf p_rel_dot_vo =
      (w_H_ee[RIGHT].block<3, 1>(0, 3) - w_H_ee[LEFT].block<3, 1>(0, 3)).transpose() * Vo.head(3);
  ...
  for (int k = 0; k < NB_ROBOTS; k++) {
    ...
    xvgp_o_d[k] = o_H_gp[k].block<3, 1>(0, 3)
        + (1.f - _go2object) * _alpha_obs[k] * Eigen::Vector3f(0.0f, 0.0f, _objectDim(2) / 2.f + 0.09f);
  ...
  // =======================================
  Eigen::Vector3f d_p_abs =
      reachable_p * w_H_avp.block<3, 1>(0, 3) + (1.0f - reachable_p) * w_H_ar_stb.block<3, 1>(0, 3);
  _error_abs.head(3) = w_H_ar.block<3, 1>(0, 3) - d_p_abs;
  ...
  Eigen::Matrix3f d_R_abs =
      reachable_p * w_H_avp.block<3, 3>(0, 0) + (1.0f - reachable_p) * w_H_ar_stb.block<3, 3>(0, 0);
  Eigen::Matrix4f w_H_ar_t = w_H_ar;
  w_H_ar_t.block<3, 3>(0, 0) =
      Utils<float>::getCombinedRotationMatrix(cpl_abs, w_H_ar.block<3, 3>(0, 0), d_R_abs);// desired
  // relative transformation
  ...
  // Coupling the orientation with the position error
  Eigen::Matrix3f d_R_rel =
      reachable_p * (coord_abs * lp_H_rvp.block<3, 3>(0, 0) + (1.0f - coord_abs) * lp_H_rp_pgrasp.block<3, 3>(0, 0))
      + (1.0f - reachable_p) * lr_H_rr_stb.block<3, 3>(0, 0);
  ...
  Eigen::Vector3f o_error_pos_abs_paral = Eigen::Vector3f(o_error_pos_abs(0), 0.0f, o_error_pos_abs(2));
  // float cp_ap = computeCouplingFactor(o_error_pos_abs_paral, 50.0f, 0.06f, 1.0f, true);  // 50.0f, 0.12f, 1.0f
  // (0.04f) (0.02f)
  ...
  // *lp_H_rp_pgrasp.block<3,1>(0,3)) + (1.0f-reachable_p) * lr_H_rr_stb.block<3,1>(0,3); // TBC
  Eigen::Vector3f d_p_rel =
      cpl_rel * (cp_ap * lp_H_rvp.block<3, 1>(0, 3) + (1.0f - cp_ap) * lp_H_rp_pgrasp.block<3, 1>(0, 3))
      + (1.0f - cpl_rel) * lr_H_rr_stb.block<3, 1>(0, 3);// TBC
...
Eigen::Vector3f dualArmFreeMotionController::compute_modulated_motion(float activation, Eigen::Matrix3f BasisQ,
  ...
  Lambda.block<1, 1>(0, 0) = activation * (BasisQ.col(0).transpose() * Amodul_ee_tang * Beta_j(0))
...
void dualArmFreeMotionController::dual_arm_motion(Eigen::Matrix4f w_H_ee[], Vector6f Vee[], Eigen::Matrix4f w_H_gp[],
  ...
  // Modulation term
  a_proximity_ = 0.5f
      * (0.5f * (std::tanh(this->sw_proxim_ * (0.5f * this->rho_ - dist2reach[LEFT])) + 1.0f)
         +// scalar function of 3D distance  to
         0.5f
             * (std::tanh(this->sw_proxim_ * (0.5f * this->rho_ - dist2reach[RIGHT]))
                + 1.0f));// initial (pre-modulation) position of attractor
  // a_proximity_  *= (1.0f-a_retract_);
  // a_proximity_ = 1.0;
  a_normal_ = a_proximity_ * 0.5f
      * (0.5f * (std::tanh(this->sw_norm_ * (this->range_norm_ - dist2line[LEFT])) + 1.0f)
         +// scalar function of distance to the line of direction VdImp
         0.5f
             * (std::tanh(this->sw_norm_ * (this->range_norm_ - dist2line[RIGHT]))
                + 1.0f));// and passing through the release position

  a_tangent_ = a_proximity_ * 0.5f
      * (0.5f * (std::tanh(this->sw_tang_ * (this->range_tang_ - dist2end[LEFT])) + 1.0f)
         +// scalar function of distance to the
         0.5f
             * (std::tanh(this->sw_tang_ * (this->range_tang_ - dist2end[RIGHT]))
                + 1.0f));// stopping position of the throwing task
  ...
  std::cout << " POSITION : ---------- [w_H_gp_t] \t" << w_H_gp_t[LEFT].block<3, 1>(0, 3).transpose() << " and \t"
            << w_H_gp_t[RIGHT].block<3, 1>(0, 3).transpose() << std::endl;
  ...
  Vector6f Amodul_ee_norm = Eigen::VectorXf::Zero(
      6);// A_prime*(X_dual - Xb_dual);          // Modulated DS that aligned  the EE with the desired velocity
  ...
  switch (taskType) {
    ...
    case 1: {// point to point motion of the object
      ...
      X_bi.head(3) =
          0.5f * (X[LEFT] + X[RIGHT]);// w_H_Do.block<3,1>(0,3) - w_H_o.block<3,1>(0,3)+ 0.5f*(X[LEFT] + X[RIGHT]);
      X_bi.tail(3) = 0.95f * (X[RIGHT] - X[LEFT]);

      Xstar_dual = _Tbi.inverse() * X_bi;
      // Amodul_ee_norm = _Tbi.inverse() * A * _Tbi *(X_dual - Xstar_dual);           // Modulated DS that aligned  the
      // EE with the desired velocity Amodul_ee_tang = _Tbi.inverse() * A * _Tbi *(X_dual - Xstar_dual);           // ;
      // --------------------------------------------------------------------------------------
    ...
    case 4: {// point to point motion of the object
      ...
      Vo_place.head(3) =
          Vo_place.head(3).normalized() * (cp_obj * Vo_place.head(3).norm() + (1.0f - cp_obj) * Vd_o.head(3).norm());
      // if(false){ // false
      //   Vo_place.head(3) = Vo_place.head(3).normalized() * (cp_obj*Vo_place.head(3).norm() +(1.0f
      //   -cp_obj)*Vd_o.head(3).norm());
      // }
      // else{
      ...
      // v_task_bi.head(3) = v_task_bi.head(3).normalized()*(cp_obj*v_task_bi.head(3).norm() +(1.0f
      // -cp_obj)*Vd_o.head(3).norm()); v_task_bi.head(3) = v_task_bi.head(3).normalized()*(v_task_bi.head(3).norm());
      v_task_bi.head(3) = v_task_bi.head(3).normalized() * Vd_o.head(3).norm();
  ...
  DS_ee_modulated =
      this->compute_modulated_motion_dual(activation, BasisQ, DS_ee_nominal, Amodul_ee_norm, Amodul_ee_tang);
  ...
  if (_modulated_reaching) {
    ...
    _refVreach[LEFT] = (1.0f - alp) * _refVreach[LEFT]
        + alp * ((1.0f - cp_ap) * DS_ee_modulated.head(3).norm() + cp_ap * VdImp[LEFT].norm());
    _refVreach[RIGHT] = (1.0f - alp) * _refVreach[RIGHT]
        + alp * ((1.0f - cp_ap) * DS_ee_modulated.tail(3).norm() + cp_ap * VdImp[RIGHT].norm());
    ...
    _refVreach[LEFT] = (1.0f - alp) * _refVreach[LEFT]
        + alp * ((1.0f - cp_ap) * DS_ee_nominal.head(3).norm() + cp_ap * VdImp[LEFT].norm());
    _refVreach[RIGHT] = (1.0f - alp) * _refVreach[RIGHT]
        + alp * ((1.0f - cp_ap) * DS_ee_nominal.tail(3).norm() + cp_ap * VdImp[RIGHT].norm());
    speed_ee[LEFT] = _refVreach[LEFT];
    speed_ee[RIGHT] = _refVreach[RIGHT];
...
Eigen::Vector3f dualArmFreeMotionController::getAbsoluteTangentError(Eigen::Matrix4f w_H_o, Eigen::Matrix4f w_H_ee[],
  ...
  //
  Eigen::Vector3f error_p_abs =
      w_H_o.block(0, 3, 3, 1) - 0.5f * (w_H_ee[LEFT].block(0, 3, 3, 1) + w_H_ee[RIGHT].block(0, 3, 3, 1));

  return oSpace * error_p_abs;
}

void dualArmFreeMotionController::compute_EE_avoidance_velocity(Eigen::Matrix4f w_H_ee[],
                                                                Vector6f (&VEE_oa)[NB_ROBOTS]) {

  Eigen::Vector3f center_ssphere_EE_l =
      w_H_ee[LEFT].block(0, 3, 3, 1) + (0.5f * _min_dist_EE - _safe_radius) * w_H_ee[LEFT].block(0, 0, 3, 3).col(2);
  Eigen::Vector3f center_ssphere_EE_r =
      w_H_ee[RIGHT].block(0, 3, 3, 1) + (0.5f * _min_dist_EE - _safe_radius) * w_H_ee[RIGHT].block(0, 0, 3, 3).col(2);
  ...
  VEE_oa[LEFT].head(3) =
      -alpha_active * _v_max * (w_H_ee[RIGHT].block(0, 3, 3, 1) - w_H_ee[LEFT].block(0, 3, 3, 1)).normalized();
  VEE_oa[RIGHT].head(3) =
...
void dualArmFreeMotionController::constrained_ang_vel_correction(Eigen::Matrix4f w_H_ee[], Eigen::Matrix4f w_H_gp[],
  ...
  // 3D Orientation Jacobian
  Eigen::Matrix3f jacMuTheta_o =
      Utils<float>::getMuThetaJacobian(do_H_o.block<3, 3>(0, 0)) * w_H_o.block<3, 3>(0, 0).transpose();
  //
  // ---------------------------------
  // computing of desired ee velocity
  // ---------------------------------
  ...
  for (int k = 0; k < NB_ROBOTS; k++) {
    ...
    Eigen::Vector3f tog = w_H_o.block<3, 1>(0, 3) - w_H_gp[k].block<3, 1>(0, 3);
    Eigen::Matrix3f skew_Mx_og;
    skew_Mx_og << 0.0f, -tog(2), tog(1), tog(2), 0.0f, -tog(0), -tog(1), tog(0), 0.0f;
    ...
    std::cout << " WWWWWWWWWWWWWWWWWWWWWWWWW Delta_V(Wo) " << k << " is \t "
              << (skew_Mx_og * Omega_object_d_).transpose() << std::endl;
...
void dualArmFreeMotionController::updateDesiredGraspingPoints(
  ...
  w_H_Dgp[RIGHT].block(0, 0, 3, 3) =
      w_H_DesObj.block(0, 0, 3, 3) * Utils<float>::pose2HomoMx(xgp_o[RIGHT], qgp_o[RIGHT]).block(0, 0, 3, 3);
  //
  ...
  if (!no_dual_mds_method) {
    w_H_Dgp[LEFT].block(0, 0, 3, 3) =
        w_H_o.block(0, 0, 3, 3) * Utils<float>::pose2HomoMx(xgp_o[LEFT], qgp_o[LEFT]).block(0, 0, 3, 3);
    w_H_Dgp[RIGHT].block(0, 0, 3, 3) =
        w_H_o.block(0, 0, 3, 3) * Utils<float>::pose2HomoMx(xgp_o[RIGHT], qgp_o[RIGHT]).block(0, 0, 3, 3);
    if (isClose2Release) {
      w_H_Dgp[LEFT] = w_H_o * o_H_ee[LEFT];
      w_H_Dgp[RIGHT] = w_H_o * o_H_ee[RIGHT];
    }
...
void dualArmFreeMotionController::getDesiredMotion(
  ...
  } else// Free-motion: reaching
  {
    if (false || no_dual_mds_method) {
      ...
      Vd_ee[LEFT].head(3) = Vd_ee[LEFT].head(3)
          + w_H_gp[LEFT].block(0, 0, 3, 3).col(2) * cp_ap * 0.20f;//     n[k] = _w_H_gp[k].block(0,0,3,3).col(2);
...
Eigen::Vector2f dualArmFreeMotionController::estimateRobot_PathLength_AverageSpeed(
  ...
  while ((!isReleasePositionReached) && (pred_count < max_horizon)) {
    ...
    Eigen::Vector3f lh_er = w_H_gpts[LEFT].block<3, 3>(0, 0).transpose()
        * (w_H_gpts[LEFT].block<3, 1>(0, 3) - w_H_EE[LEFT].block<3, 1>(0, 3));//
    Eigen::Vector3f rh_er = w_H_gpts[RIGHT].block<3, 3>(0, 0).transpose()
        * (w_H_gpts[RIGHT].block<3, 1>(0, 3) - w_H_EE[RIGHT].block<3, 1>(0, 3));//
...
void dualArmFreeMotionController::getCoordinatedTranslation(Eigen::Vector3f x_ee[], Eigen::Vector3f x_gp[],
  ...
  Eigen::Vector3f d_p_rel =
      cpl_rel * (cp_ap * x_rel_gp + (1.0f - cp_ap) * x_rel_pgrasp) + (1.0f - cpl_rel) * x_rel_stb;// TBC
...
Eigen::MatrixXf dualArmFreeMotionController::get_bimanual_grasp_mx(const Eigen::Matrix4f& w_H_o,
                                                                   Eigen::Matrix4f w_H_gp[]) {
  ...
  for (int k = 0; k < 2; k++) {
    ...
    //
    skew_Mx_og[k] << 0.0f, -tog[k](2), tog[k](1), tog[k](2), 0.0f, -tog[k](0), -tog[k](1), tog[k](0), 0.0f;
  }
  //
...
Vector6f dualArmFreeMotionController::compute_desired_task_twist(const Eigen::Matrix4f& w_H_c,
                                                                 const Eigen::Matrix4f& w_H_d) {
  //
  Vector6f error_ee;
  error_ee.setZero();
  Vector6f des_twist_ee;
  des_twist_ee.setZero();
  //
