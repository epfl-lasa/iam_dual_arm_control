...
void dualArmFreeMotionController::computeCoordinatedMotion(Eigen::Matrix4f w_H_ee[],  Eigen::Matrix4f w_H_gp[], Vector6f (&Vd_ee)[NB_ROBOTS], Eigen::Vector4f (&qd)[NB_ROBOTS])
{
  ...
  Eigen::Matrix4f w_H_ar_std, lr_H_rr_std;   // absolute and relative EE standby poses
  ...
  Utils<float>::getBimanualTransforms(this->_w_H_eeStandby[LEFT], this->_w_H_eeStandby[RIGHT], w_H_ar_std, lr_H_rr_std); // standby arms
  //
  ...
  // =======================================
  Eigen::Vector3f d_p_abs = reachable_p *w_H_ap.block<3,1>(0,3) + (1.0f-reachable_p)*w_H_ar_std.block<3,1>(0,3);
  ...
  Eigen::Matrix3f d_R_abs = reachable_p *w_H_ap.block<3,3>(0,0) + (1.0f-reachable_p)*w_H_ar_std.block<3,3>(0,0);
  ...
  Eigen::Matrix3f d_R_rel = reachable_p* (coord_abs*lp_H_rp.block<3,3>(0,0) + (1.0f-coord_abs)*lp_H_rp_pgrasp.block<3,3>(0,0)) 
                   				+ (1.0f- reachable_p)*lr_H_rr_std.block<3,3>(0,0);
  ...
  Eigen::Vector3f d_p_rel = reachable_p *(cpl_rel * lp_H_rp.block<3,1>(0,3) + (1.0f-cpl_rel) *lp_H_rp_pgrasp.block<3,1>(0,3))
                   				+ (1.0f-reachable_p) * lr_H_rr_std.block<3,1>(0,3); // TBC  
  ...
  this->computeDesiredOrientation(coord_abs, w_H_ee, w_H_gp, qd);
}


void dualArmFreeMotionController::computeConstrainedMotion(Eigen::Matrix4f w_H_ee[],  Eigen::Matrix4f w_H_gp[], Vector6f (&Vd_ee)[NB_ROBOTS], Eigen::Vector4f (&qd)[NB_ROBOTS])
{
  // get the task transformations
  Eigen::Matrix4f w_H_ar, lr_H_rr;           // absolute and relative EE poses
  Eigen::Matrix4f w_H_ap, lp_H_rp;           // absolute and relative object's grasp points
  ...
  // computing the velocity
  _V_rel.head(3) = -2.5f*gain_p_rel * _error_rel.head(3);
  _V_rel.tail(3) = -2.5f*jacMuTheta_rel.inverse() * gain_o_rel * _error_rel.tail(3);
...
void dualArmFreeMotionController::computeAsyncMotion(Eigen::Matrix4f w_H_ee[],  Eigen::Matrix4f w_H_gp[], Vector6f (&Vd_ee)[NB_ROBOTS], Eigen::Vector4f (&qd)[NB_ROBOTS])
{
	//
	for(int k=0; k<NB_ROBOTS; k++)
	{
 	...
 	// Computation of desired orientation
  this->computeDesiredOrientation(0.5f, w_H_ee, w_H_gp, qd);
}

void dualArmFreeMotionController::computeDesiredOrientation(float weight, Eigen::Matrix4f w_H_ee[],  Eigen::Matrix4f w_H_gp[], Eigen::Vector4f (&qd)[NB_ROBOTS])
{
	for(int k = 0; k<NB_ROBOTS; k++)
	{
