...
bool dualArmCooperativeController::init() {
  ...
  _min_Fz = 30.0f;// 30.0 20.0;  // 40.0f;
  _min_nF = 30.0f;// 30.0 20.0;  // 40.0f;
  _max_nF = 45.0f;// 45.0f; //  60.0f;
  //
  _mu_ee = 0.7f;     // 0.9f; //0.4f;
  _gamma_ee = 0.7f;  // 0.9f; //0.4f;
...
void dualArmCooperativeController::getGraspKineDynVariables(Eigen::Matrix4f w_H_o, Eigen::Matrix4f w_H_ee[],
...
void dualArmCooperativeController::check_contact_proximity(Eigen::Matrix4f w_H_ee[], Eigen::Matrix4f w_H_cp[],
  ...
  Eigen::Vector3f lh_er =
      w_H_cp[LEFT].block<3, 3>(0, 0).transpose() * (w_H_cp[LEFT].block<3, 1>(0, 3) - w_H_ee[LEFT].block<3, 1>(0, 3));//
  Eigen::Vector3f rh_er = w_H_cp[RIGHT].block<3, 3>(0, 0).transpose()
      * (w_H_cp[RIGHT].block<3, 1>(0, 3) - w_H_ee[RIGHT].block<3, 1>(0, 3));//
  ...
  // if(_contactOccured && (fabs(_dist2contact[RIGHT]) <= _tol_dist2contact) && (fabs(_dist2contact[LEFT]) <=
  // _tol_dist2contact)){
...
bool dualArmCooperativeController::computeBimanualGraspMatrix(Eigen::Matrix4f w_H_o, Eigen::Matrix4f w_H_ee[],
                                                              Eigen::Matrix<float, 6, 12>& GraspMxHands_) {
  //
...
void dualArmCooperativeController::thresholdNormalForcesEEs(float min, float max, Eigen::Matrix3f w_R_h,
                                                            Vector6f& Wrench_w) {
  // Thresholding normal forces
  Vector6f Wrench_h;
...
//
bool dualArmCooperativeController::getComplementaryConstraints(Matrix6f world_Xstar_desEE[], float dist2contact[],
                                                               float tol_dist2contact) {
...
bool dualArmCooperativeController::getContactConstraints(Matrix6f world_Xstar_EE[]) {
  for (int k = 0; k < NB_ROBOTS; k++) {
    ...
    _contactConstraintMatrix[k].block<1, 6>(1, 0) =
        -_mu_ee / sqrt(2.0f) * ee_X_world.block<1, 6>(2, 0) - ee_X_world.block<1, 6>(0, 0);
    _contactConstraintMatrix[k].block<1, 6>(2, 0) =
        -_mu_ee / sqrt(2.0f) * ee_X_world.block<1, 6>(2, 0) + ee_X_world.block<1, 6>(0, 0);
    _contactConstraintMatrix[k].block<1, 6>(3, 0) =
        -_mu_ee / sqrt(2.0f) * ee_X_world.block<1, 6>(2, 0) - ee_X_world.block<1, 6>(1, 0);
    _contactConstraintMatrix[k].block<1, 6>(4, 0) =
        -_mu_ee / sqrt(2.0f) * ee_X_world.block<1, 6>(2, 0) + ee_X_world.block<1, 6>(1, 0);
    _contactConstraintMatrix[k].block<1, 6>(5, 0) =
        -_gamma_ee * ee_X_world.block<1, 6>(2, 0) - ee_X_world.block<1, 6>(5, 0);
    _contactConstraintMatrix[k].block<1, 6>(6, 0) =
        -_gamma_ee * ee_X_world.block<1, 6>(2, 0) + ee_X_world.block<1, 6>(5, 0);
    _contactConstraintMatrix[k].block<1, 6>(7, 0) =
        -_deltaX_ee * ee_X_world.block<1, 6>(2, 0) - ee_X_world.block<1, 6>(4, 0);
    _contactConstraintMatrix[k].block<1, 6>(8, 0) =
        -_deltaX_ee * ee_X_world.block<1, 6>(2, 0) + ee_X_world.block<1, 6>(4, 0);
    _contactConstraintMatrix[k].block<1, 6>(9, 0) =
        -_deltaY_ee * ee_X_world.block<1, 6>(2, 0) - ee_X_world.block<1, 6>(3, 0);
    _contactConstraintMatrix[k].block<1, 6>(10, 0) =
        -_deltaY_ee * ee_X_world.block<1, 6>(2, 0) + ee_X_world.block<1, 6>(3, 0);
...
void dualArmCooperativeController::load_wrench_data(Vector6f desired_object_wrench_) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    bwc_params.CLH_1[i] = _contactConstraintMatrix[LEFT](0, i);
    bwc_params.CLH_7[i] = _contactConstraintMatrix[LEFT](6, i);
    bwc_params.CLH_2[i] = _contactConstraintMatrix[LEFT](1, i);
    bwc_params.CLH_8[i] = _contactConstraintMatrix[LEFT](7, i);
    bwc_params.CLH_3[i] = _contactConstraintMatrix[LEFT](2, i);
    bwc_params.CLH_9[i] = _contactConstraintMatrix[LEFT](8, i);
    bwc_params.CLH_4[i] = _contactConstraintMatrix[LEFT](3, i);
    bwc_params.CLH_10[i] = _contactConstraintMatrix[LEFT](9, i);
    bwc_params.CLH_5[i] = _contactConstraintMatrix[LEFT](4, i);
    bwc_params.CLH_11[i] = _contactConstraintMatrix[LEFT](10, i);
    bwc_params.CLH_6[i] = _contactConstraintMatrix[LEFT](5, i);

    bwc_params.CRH_1[i] = _contactConstraintMatrix[RIGHT](0, i);
    bwc_params.CRH_7[i] = _contactConstraintMatrix[RIGHT](6, i);
    bwc_params.CRH_2[i] = _contactConstraintMatrix[RIGHT](1, i);
    bwc_params.CRH_8[i] = _contactConstraintMatrix[RIGHT](7, i);
    bwc_params.CRH_3[i] = _contactConstraintMatrix[RIGHT](2, i);
    bwc_params.CRH_9[i] = _contactConstraintMatrix[RIGHT](8, i);
    bwc_params.CRH_4[i] = _contactConstraintMatrix[RIGHT](3, i);
    bwc_params.CRH_10[i] = _contactConstraintMatrix[RIGHT](9, i);
    bwc_params.CRH_5[i] = _contactConstraintMatrix[RIGHT](4, i);
    bwc_params.CRH_11[i] = _contactConstraintMatrix[RIGHT](10, i);
...
void dualArmCooperativeController::computeControlWrench(Eigen::Matrix4f w_H_o, Eigen::Matrix4f w_H_ee[],
                                                        Eigen::Matrix4f w_H_cp[], Vector6f desired_object_wrench_,
                                                        bool isForceDetected) {
  //
...
//
void dualArmCooperativeController::getPredefinedContactForceProfile(bool goHome, int contactState,
                                                                    Eigen::Matrix4f w_H_o, Eigen::Matrix4f w_H_ee[],
                                                                    Eigen::Matrix4f w_H_cp[], bool isForceDetected) {
  //
  this->getGraspKineDynVariables(w_H_o, w_H_ee, w_H_cp);
  this->check_contact_proximity(w_H_ee, w_H_cp, isForceDetected);
  //
...
void dualArmCooperativeController::getPredefinedContactForceProfile(bool goHome, int contactState,
                                                                    Eigen::Matrix4f w_H_o, Eigen::Matrix4f w_H_ee[],
                                                                    Eigen::Matrix4f w_H_cp[], float object_mass,
                                                                    bool isForceDetected) {
  //
  this->getGraspKineDynVariables(w_H_o, w_H_ee, w_H_cp);
  this->check_contact_proximity(w_H_ee, w_H_cp, isForceDetected);
  ...
  if (goHome) {
    ...
    if (contactState == CONTACT) {
      _f_applied[LEFT].head(3) = _targetForce * _nC[LEFT] + 0.5f * object_mass * Eigen::Vector3f(0.0f, 0.0f, 9.81f);
      _f_applied[RIGHT].head(3) = _targetForce * _nC[RIGHT] + 0.5f * object_mass * Eigen::Vector3f(0.0f, 0.0f, 9.81f);
    } else if (contactState == CLOSE_TO_CONTACT) {
      _f_applied[LEFT].head(3) = 3.0f * _nC[LEFT];
      _f_applied[RIGHT].head(3) = 3.0f * _nC[RIGHT];
    } else {
...
void dualArmCooperativeController::getAppliedWrenches(bool goHome, int contactState, Eigen::Matrix4f w_H_o,
                                                      Eigen::Matrix4f w_H_ee[], Eigen::Matrix4f w_H_cp[],
                                                      Vector6f desired_object_wrench, float object_mass,
                                                      bool qp_wrench_generation, bool isForceDetected) {
