...
void dualArmCooperativeController::getGraspKineDynVariables(Eigen::Matrix4f w_H_o,
...
void dualArmCooperativeController::check_contact_proximity(Eigen::Matrix4f w_H_ee[],
                                                           Eigen::Matrix4f w_H_cp[],
...
bool dualArmCooperativeController::computeBimanualGraspMatrix(Eigen::Matrix4f w_H_o,
                                                              Eigen::Matrix4f w_H_ee[],
...
//
bool dualArmCooperativeController::getComplementaryConstraints(Matrix6f world_Xstar_desEE[],
                                                               float dist2contact[],
  ...
  for (int k = 0; k < NB_ROBOTS; k++) {
    ...
    if (fabs(dist2contact[k]) <= tol_dist2contact) { thresh_dist2cnt[k] = 0.0; }
...
void dualArmCooperativeController::computeOptimalWrench(Vector6f desired_object_wrench_) {
  ...
  // ------------------------------------
  // contact forces
  for (int i = 0; i < 12; i++) { _optimal_contact_wrench_EEs(i) = bwc_vars.Fh[i]; }
  // slack variables
  for (int i = 0; i < 6; i++) { _optimal_slack(i) = bwc_vars.w[i]; }
  //
...
void dualArmCooperativeController::load_wrench_data(Vector6f desired_object_wrench_) {
  ...
  // weight hands wrench
  for (int i = 0; i < 12; i++) { bwc_params.QFh[i] = _weight_EEs_wrench(i); }
  // weight hands slack
  for (int i = 0; i < 6; i++) { bwc_params.Qw[i] = _weight_EEs_slack(i); }
  ...
  for (int i = 0; i < 6; i++) { bwc_params.b1[i] = desired_object_wrench_(i); }
...
void dualArmCooperativeController::computeControlWrench(Eigen::Matrix4f w_H_o,
                                                        Eigen::Matrix4f w_H_ee[],
                                                        Eigen::Matrix4f w_H_cp[],
