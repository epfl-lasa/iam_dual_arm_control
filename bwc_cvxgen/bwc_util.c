...
float bwc_randn_internal(long *idum, int reset) {
  ...
  if (iset==0) {
    do {
      v1 = 2.0*bwc_ran1(idum, reset)-1.0;
      v2 = 2.0*bwc_ran1(idum, reset)-1.0;
      rsq = v1*v1+v2*v2;
...
double bwc_randn(void) {
  return bwc_randn_internal(&bwc_global_seed, 0);
}
void bwc_reset_rand(void) {
  srand(15);
  bwc_global_seed = 1;
  bwc_randn_internal(&bwc_global_seed, 1);
}
